import type { WorktreeInfo, WorktreeListRow } from './types.ts'
...
export function filterWorktrees(worktrees: WorktreeInfo[], options: FilterWorktreesOptions): WorktreeInfo[] {
  return worktrees.filter(
    (worktree) => (options.showArchived || !worktree.archived) && matchesWorktreeSearch(worktree, options.query),
  )
}
...
export function countAgentStatusesIn(
  ...
  notifiedBranches: ReadonlySet<string> = new Set(),
...
export function branchesWithAgentStatus(
  ...
  return rows
    .filter(
      ({ worktree }) =>
        overflowStatusOf(worktree, notifiedBranches) === status && (!branches || branches.has(worktree.branch)),
    )
