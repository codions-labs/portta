...
import {
  type AgentsUiConversationEvent,
  AgentsUiConversationEventSchema,
  type AgentsUiConversationMessage,
  type AgentsUiWorktreeConversationResponse,
  apiPaths,
...
import { createLinearIssue, fetchTeamByKey, type LinearIssue } from 'portta-host/taskflow/services/linear-service'
import { findDuplicateLinearIssue, polishLinearIssueTitle } from 'portta-host/taskflow/services/linear-title-service'
import WebSocket from 'ws'
import { daemonBaseUrl, daemonHeaders, daemonSocketUrl, flowApi } from './daemon.ts'
import { flowInvocation } from './flow-action.ts'
import { CommandUsageError, formatServerError, parseEnvOverrides, resolveProjectBaseUrl } from './shared.ts'

// The daemon serves each project under `/api/modules/taskflow/<prefix>`.
// `runOneshot` resolves the scoped base once and stashes it here so every helper
// (api calls + the agents WebSocket) targets the right project. Null → the
// module's unscoped base (port only).
let oneshotBaseUrl: string | null = null
function oneshotApi(port: number): ReturnType<typeof flowApi> {
  return flowApi(oneshotBaseUrl ?? daemonBaseUrl(port))
}
...
export interface OneshotCliOptions {
  resume?: string
  prompt?: string
  agent?: string
  base?: string
...
export function oneshotFromCli(branchArg: string | undefined, options: OneshotCliOptions): ParsedOneshotCommand {
  const body: CreateWorktreeRequest = {}
  let branch: string | null = branchArg ?? null
  const branchFlagUsed = options.branch !== undefined
  const prompt = options.prompt ?? null
  const resume = options.resume !== undefined
  const resumeBranch = options.resume?.trim() ?? null
  let fromLinearIssueId: string | null = null
  let postToLinearTarget: PostWorktreeToLinearTarget | null = null

  if (options.agent !== undefined) body.agent = options.agent.trim()
  if (options.base !== undefined) body.baseBranch = options.base
  if (options.profile !== undefined) body.profile = options.profile

  if (options.linear !== undefined) {
    const target = parseLinearTarget(options.linear)
  ...
  if (options.branch !== undefined) {
    if (branch && branch !== options.branch) {
      throw new CommandUsageError(`Conflicting branch values: "${branch}" and "${options.branch}"`)
    }
  ...
  if (branchFlagUsed && !fromLinearIssueId) {
    throw new CommandUsageError(
      '--branch only applies with --linear; pass the branch as a positional argument otherwise',
    )
  }
  ...
  if (envOverrides) body.envOverrides = envOverrides
...
function isSameStreamingMessage(state: ConversationPrintState, message: AgentsUiConversationMessage): boolean {
  ...
  return (
    message.turnId === state.streamingTurnId &&
    (state.streamingText.startsWith(message.text) || message.text.startsWith(state.streamingText))
  )
}

function printNewMessages(state: ConversationPrintState, messages: AgentsUiConversationMessage[]): void {
...
function streamConversation(
  ...
  const connect = (): void => {
    if (closed) return
    const url = daemonSocketUrl(
      oneshotBaseUrl ?? daemonBaseUrl(port),
      apiPaths.streamAgentsWorktreeConversation.replace(':name', encodeURIComponent(branch)),
    )
    const ws = new WebSocket(url, { headers: daemonHeaders() })
    socket = ws
    ...
    ws.addEventListener('close', () => {
      ...
      if (RECONNECT_WARN_AT.includes(consecutiveFailures)) {
        stderr(
          `[${timestamp()}] [warn] ${APP_NAME} server unreachable, retrying (${consecutiveFailures}/${MAX_CONSECUTIVE_RECONNECTS})`,
        )
      }
      if (consecutiveFailures >= MAX_CONSECUTIVE_RECONNECTS) {
        closed = true
        onFatal(`${APP_NAME} server unreachable after ${consecutiveFailures} reconnect attempts`)
        return
      }
...
function recordPrEvents(state: PollState, worktree: ProjectWorktreeSnapshot, onPrEvent: (line: string) => void): void {
...
async function ensureWorktreeReady(
  ...
  while (Date.now() < deadline) {
    try {
      ...
      if (worktree?.mux && worktree.status !== 'creating' && worktree.status !== 'closed') {
...
function printConversationHistory(initial: AgentsUiWorktreeConversationResponse, state: ConversationPrintState): void {
 ...
 * deltas.
 */
function pollConversationHistory(branch: string, port: number, state: ConversationPrintState): { stop: () => void } {
  const api = oneshotApi(port)
  let stopped = false
  let timer: ReturnType<typeof setTimeout> | null = null
...
export function oneshotConversationTransport(agentName: string | null | undefined): 'history' | 'stream-and-history' {
  return agentName === 'codex' ? 'history' : 'stream-and-history'
}

type DuplicateChoice = 'use_existing' | 'create_new' | 'cancel'

async function promptDuplicateChoice(candidate: LinearIssue, polishedTitle: string): Promise<DuplicateChoice> {
  ...
  p.note(`${candidate.identifier}: ${candidate.title}\n${candidate.url}`, 'Possible existing match')
  const choice = await p.select<DuplicateChoice>({
    message: `Found a possible existing match. What should ${APP_NAME} do?`,
  ...
  return choice as DuplicateChoice
}
...
export async function runOneshot(parsed: ParsedOneshotCommand, port: number): Promise<number> {
  ...
  try {
    ...
    if (postToLinearTarget) {
      ...
      if (projectAutoName.linearAvailability === 'missing_api_key') {
        stderr(
          `[${timestamp()}] [error] server has no LINEAR_API_KEY — the post-back to Linear at the end of the run will fail. Set the env var on the ${APP_NAME} server and restart it.`,
        )
        return 1
      }
      if (projectAutoName.linearAvailability === 'disabled') {
        stderr(`[${timestamp()}] [error] Linear integration is disabled on the ${APP_NAME} server.`)
        return 1
      }
    ...
    // to stay consistent with `taskflow add --from-linear`. The server still
    ...
    if (postToLinearTarget?.kind === 'team') {
      if (!parsed.prompt) {
        stderr(
          `[${timestamp()}] [error] --linear ${postToLinearTarget.teamKey} requires --prompt to derive an issue title`,
        )
        return 1
      }
      ...
      if (parsed.resume) {
        ...
        stdout(
          `[${timestamp()}] [event] no Linear issue for this resume; creating a fresh ${postToLinearTarget.teamKey}-N for the post-back`,
        )
      }
    ...
    if (fromLinearIssueId) {
      ...
      stdout(
        `[${timestamp()}] [event] seed source: ${seed.source}${seed.branch ? ` branch=${seed.branch}` : ''}${seed.prUrl ? ` pr=${seed.prUrl}` : ''}`,
      )
      ...
      // Use "existing" mode when the seed pointed to an existing branch (taskflow session or open PR).
  ...
  const conversationTransport = oneshotConversationTransport(ready.worktree.agentName)
  if (conversationTransport === 'stream-and-history') {
    stream = streamConversation(branch, port, conversationState, stderr, (reason) => {
      stderr(`[${timestamp()}] [fatal] ${reason}`)
      finalize(1)
    })
  }
  // The Codex TUI owns the thread writer during an oneshot, so its transcript is
  // read from the native session log. Other agents retain WS delivery with the
  // same history poll as a fallback for terminal-routed messages.
  ...
  const onSignal = (): void => {
    ...
    stdout(`[${timestamp()}] [event] resume with: ${flowInvocation()} oneshot --resume ${branch} --prompt <text>`)
    finalize(130)
  }
  ...
  // watcher (see packages/host/src/modules/taskflow/services/oneshot-watcher-service.ts). The CLI
