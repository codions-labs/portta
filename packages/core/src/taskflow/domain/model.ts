import type { ENV_NAMES } from '../config.ts'
import type { AgentId, RuntimeKind } from './config.ts'
...
export type WorktreeConversationMeta = CodexWorktreeConversationMeta | ClaudeWorktreeConversationMeta
...
/** Linear post-back target embedded in oneshot meta. The contract's
 *  PostWorktreeToLinearTarget schema is checked against this shape, so meta and
 *  wire shapes stay in lockstep. */
export type OneshotPostTarget = { kind: 'issue'; issueId: string } | { kind: 'team'; teamKey: string; title?: string }
...
export interface WorktreeMeta {
  ...
  agent: AgentId
  runtime: RuntimeKind
  /** Presentation selected when this session was last opened. Older metadata defaults to terminal. */
  interfaceMode?: import('./config.ts').SessionInterfaceMode
  startupEnvValues: Record<string, string>
  allocatedPorts: Record<string, number>
  source?: WorktreeSource
  oneshot?: OneshotMeta
  /** The issue this worktree is for, when one started it. A ref, never a copy. */
  issueRef?: string
 ...
 *  `taskflow restore` can re-open them after a server restart or reboot. */
...
export type ControlEnvMap = Record<
  typeof ENV_NAMES.controlUrl | typeof ENV_NAMES.controlToken | typeof ENV_NAMES.worktreeId | typeof ENV_NAMES.branch,
  string
>
