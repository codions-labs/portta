import { APP_SLUG } from '../config.ts'
import type { ServiceSpec } from './config.ts'
import type { PrEntry, WorktreeMeta } from './model.ts'
 ...
 *  shadow them. */
export const RESERVED_PROJECT_PREFIXES: ReadonlySet<string> = new Set(['api', 'ws', 'assets', 'docs'])
...
/** Derive a taskflow project URL prefix from a project directory basename.
...
export function deriveProjectPrefix(projectDir: string, takenPrefixes: Iterable<string>): string {
  const basename = projectDir.replace(/\/+$/, '').split('/').pop() ?? APP_SLUG
  const base = sanitizeProjectPrefix(basename) || APP_SLUG
...
export function allocateServicePorts(existingMetas: WorktreeMeta[], services: ServiceSpec[]): Record<string, number> {
  ...
  if (!reference || reference.portStart == null) return {}
