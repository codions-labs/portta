...
describe('AutoNameService', () => {
  ...
  it('passes --model to claude when model is specified', async () => {
    ...
    await service.generateBranchName({ provider: 'claude', model: 'haiku' }, 'Add search')
  ...
  it('spawns codex exec with correct args', async () => {
    ...
    expect(calls[0]).toEqual([
      'codex',
      '-c',
      'developer_instructions="Generate a branch name"',
      'exec',
      '--ephemeral',
  ...
  it('passes -m to codex when model is specified', async () => {
    ...
    await service.generateBranchName({ provider: 'codex', model: 'gpt-4.1' }, 'Add bulk actions')
  ...
  it('omits -m from codex when model is not specified', async () => {
    ...
    await service.generateBranchName({ provider: 'codex' }, 'Add bulk actions')
  ...
  it('normalizes messy output into a valid branch name', async () => {
    ...
    const branch = await service.generateBranchName({ provider: 'claude' }, 'Fix login')
  ...
  it('throws when CLI is not found', async () => {
    ...
    await expect(service.generateBranchName({ provider: 'claude' }, 'Fix bug')).rejects.toThrow(
  ...
  it('throws on non-zero exit code', async () => {
    ...
    await expect(service.generateBranchName({ provider: 'codex' }, 'Fix bug')).rejects.toThrow(
      /codex failed \(command: .*\): authentication required/,
    )
  })

  it('returns a change-prefixed fallback branch when the real spawn path times out', async () => {
    const tempDir = await mkdtemp(join(tmpdir(), 'taskflow-auto-name-'))
    const claudePath = join(tempDir, 'claude')
    await nodeTest.write(claudePath, "#!/bin/sh\ntrap '' TERM\nwhile true; do sleep 1; done\n")
    await chmod(claudePath, 0o755)

    const originalPath = nodeTest.env.PATH
    const timeoutMs = 50
    const deadlineMs = 1_000
    nodeTest.env.PATH = originalPath ? `${tempDir}:${originalPath}` : tempDir
    process.env.PATH = nodeTest.env.PATH

    try {
      const startedAt = Date.now()
      const branch = await Promise.race([
        new AutoNameService({ timeoutMs }).generateBranchName({ provider: 'claude' }, 'Fix bug'),
        nodeTest.sleep(deadlineMs).then(() => {
      ...
      nodeTest.env.PATH = originalPath
  ...
  it('truncates branch names longer than 40 characters', async () => {
    ...
    const branch = await service.generateBranchName({ provider: 'claude' }, 'A very long task description')
  ...
  it('removes trailing hyphens after truncation', async () => {
    ...
    const branch = await service.generateBranchName({ provider: 'claude' }, 'Some task')
  ...
  it('escapes special characters in system prompt for codex TOML config', async () => {
    ...
    await service.generateBranchName({ provider: 'codex', systemPrompt: 'Use "kebab-case"\nNo prefixes' }, 'Fix bug')
