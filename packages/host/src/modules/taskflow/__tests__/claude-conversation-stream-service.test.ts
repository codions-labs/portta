...
import type { ClaudeCliGateway, ClaudeCliRunCallbacks, ClaudeCliRunHandle } from '../adapters/claude-cli.ts'
...
describe('ClaudeConversationStreamService', () => {
  ...
  it('replays active Claude live messages to late subscribers without sending snapshots', () => {
    ...
    expect(events.map((event) => event.type)).toEqual(['conversationStatus', 'messageUpsert', 'messageUpsert'])
    expect(events[1]).toMatchObject({
      type: 'messageUpsert',
      message: {
        id: 'claude-user:claude-turn:turn-1',
  ...
  it('replays retained completed Claude live messages to late subscribers', () => {
    ...
    expect(events.map((event) => event.type)).toEqual(['messageUpsert', 'messageUpsert', 'conversationStatus'])
    expect(events[1]).toMatchObject({
      type: 'messageUpsert',
      message: {
        id: 'msg_1:0',
