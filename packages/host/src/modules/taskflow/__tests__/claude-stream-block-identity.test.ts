import type { AgentsUiConversationEvent } from 'portta-contracts/taskflow'
...
import { AgentsConversationStreamSession } from '../services/agents-ui-stream-service.ts'
...
const TWO_MESSAGE_TURN: string[] = [
  ...
  JSON.stringify({
    type: 'stream_event',
    session_id: 'session-1',
    event: { type: 'content_block_delta', index: 0, delta: { type: 'text_delta', text: 'Let me read that file.' } },
  }),
  JSON.stringify({
    type: 'assistant',
    session_id: 'session-1',
    uuid: 'rec-A-text',
    message: { id: 'msg_AAA', role: 'assistant', content: [{ type: 'text', text: 'Let me read that file.' }] },
  }),
  ...
  JSON.stringify({
    ...
    message: {
      id: 'msg_AAA',
      role: 'assistant',
      content: [{ type: 'tool_use', id: 'toolu_1', name: 'Read', input: { file_path: '/tmp/alpha.txt' } }],
    },
  }),
  ...
  JSON.stringify({
    ...
    session_id: 'session-1',
    uuid: 'rec-toolresult',
    message: { role: 'user', content: [{ type: 'tool_result', tool_use_id: 'toolu_1', content: 'hello from alpha' }] },
  }),
  ...
  JSON.stringify({
    type: 'stream_event',
    session_id: 'session-1',
    event: { type: 'content_block_delta', index: 0, delta: { type: 'text_delta', text: 'The file says hello.' } },
  }),
  JSON.stringify({
    type: 'assistant',
    session_id: 'session-1',
    uuid: 'rec-B-text',
    message: { id: 'msg_BBB', role: 'assistant', content: [{ type: 'text', text: 'The file says hello.' }] },
  }),
  JSON.stringify({ type: 'stream_event', session_id: 'session-1', event: { type: 'content_block_stop', index: 0 } }),
  JSON.stringify({
    type: 'stream_event',
...
describe('Claude stream block identity (investigation)', () => {
  it('the parser surfaces message.id so two same-index text blocks can be told apart', () => {
    const starts = TWO_MESSAGE_TURN.map(parseClaudeStreamLine).filter((p): p is NonNullable<typeof p> => p !== null)
