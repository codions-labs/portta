...
import type { DockerProfileConfig } from '../adapters/config.ts'
import { buildDockerRunArgs, type LaunchContainerOpts } from '../adapters/docker.ts'
...
function build(opts: LaunchContainerOpts, existingPaths = new Set<string>(), sshAuthSock?: string): string[] {
  return buildDockerRunArgs(opts, existingPaths, HOME, 'taskflow-test-123', sshAuthSock, UID, GID)
}
...
describe('buildDockerRunArgs — extraMounts', () => {
  ...
  it('uses hostPath as guestPath when guestPath is omitted', () => {
    const args = build(
      makeOpts({ sandboxConfig: makeDockerProfile({ image: 'img', mounts: [{ hostPath: '/data/shared' }] }) }),
    )
...
describe('buildDockerRunArgs — SSH agent forwarding', () => {
  ...
  it('SSH_AUTH_SOCK from envPassthrough is blocked by reservedKeys', () => {
    const args = build(
      makeOpts({ sandboxConfig: makeDockerProfile({ image: 'img', envPassthrough: ['SSH_AUTH_SOCK'] }) }),
    )
