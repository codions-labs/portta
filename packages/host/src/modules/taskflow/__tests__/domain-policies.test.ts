...
describe('deriveProjectPrefix', () => {
  it('returns the basename when no collision', () => {
    expect(deriveProjectPrefix('/home/me/projects/taskflow', [])).toBe('taskflow')
    expect(deriveProjectPrefix('/srv/widgets/', [])).toBe('widgets')
  })

  it('falls back to a default when the basename has no alphanumerics', () => {
    expect(deriveProjectPrefix('/repo/...', [])).toBe('taskflow')
  })

  it('appends -2, -3, ... to avoid collisions', () => {
    expect(deriveProjectPrefix('/a/taskflow', ['taskflow'])).toBe('taskflow-2')
    expect(deriveProjectPrefix('/a/taskflow', ['taskflow', 'taskflow-2'])).toBe('taskflow-3')
  })
  ...
  it('never returns a reserved prefix even when the basename matches one', () => {
    ...
    expect(deriveProjectPrefix('/srv/docs', [])).toBe('docs-2')
  })
})
...
describe('compareWorktreeOrder', () => {
  ...
  it('ranks open worktrees above every closed one', () => {
    const rows = [worktree('closed-open-pr', { prStates: ['open'] }), worktree('open-no-pr', { open: true })]

    expect([...rows].sort(compareWorktreeOrder).map((row) => row.branch)).toEqual(['open-no-pr', 'closed-open-pr'])
  })
  ...
  it('promotes a worktree whose repos disagree on pr state if any pr is open', () => {
    ...
    expect([...rows].sort(compareWorktreeOrder).map((row) => row.branch)).toEqual(['one-open', 'all-merged'])
  })
