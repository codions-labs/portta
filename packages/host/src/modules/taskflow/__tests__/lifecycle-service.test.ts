...
import type { AutoNameConfig, ProjectConfig } from 'portta-core/taskflow'
import { afterEach, describe, expect, it } from 'vitest'
import type { AgentCapabilitiesGateway } from '../adapters/agent-capabilities.ts'
...
import { canonicalizeFsPath, type GitGateway, NodeGitGateway, sameFsPath } from '../adapters/git.ts'
...
import type { AutoNameGenerator } from '../services/auto-name-service.ts'
import {
  buildCreateWorktreeTargets,
  type CreateWorktreeProgress,
  LifecycleError,
  LifecycleService,
} from '../services/lifecycle-service.ts'
...
function run(args: string[], cwd: string): string {
  const result = nodeTest.spawnSync(args, {
    cwd,
    stdout: 'pipe',
    stderr: 'pipe',
  })
...
class FakeSessionGateway implements SessionGateway {
  ...
  readonly createdPanes: Array<{ target: string; cwd: string; command?: string }> = []
  ...
  async splitWindow(opts: {
    ...
    this.createdPanes.push({ target: opts.target, cwd: opts.cwd, ...(opts.command ? { command: opts.command } : {}) })
...
class FakeDockerGateway implements DockerGateway {
  ...
  async launchContainer(opts: LaunchContainerOpts): Promise<string> {
    ...
    return `taskflow-${opts.branch}-container`
  }
...
class FakeHookRunner implements LifecycleHookRunner {
  ...
  constructor(onRun?: (input: RunLifecycleHookInput) => void | Promise<void>) {
    this.onRun = onRun
  }
...
class FakeAutoNameService implements AutoNameGenerator {
  ...
  constructor(branch = 'generated-branch') {
    this.branch = branch
  }
...
class AheadTrackingGitGateway extends NodeGitGateway {
  private readonly branches: Set<string>
  constructor(branches: Set<string>) {
    super()
    this.branches = branches
  }

  private tracked(cwd: string): boolean {
    for (const branch of this.branches) if (cwd.endsWith(branch)) return true
    return false
  }

  override readWorktreeStatus(cwd: string): ReturnType<NodeGitGateway['readWorktreeStatus']> {
    const status = super.readWorktreeStatus(cwd)
    if (!this.tracked(cwd)) return status
    return { ...status, dirty: false, aheadCount: 2 }
  }

  override countUnsavedCommits(cwd: string, baseBranch: string): number {
    return this.tracked(cwd) ? 2 : super.countUnsavedCommits(cwd, baseBranch)
  }
}
...
function makeLifecycleService(
  ...
  git: GitGateway = new NodeGitGateway(),
  ...
  agentCapabilities: AgentCapabilitiesGateway = { checkCodexAutoReview: async () => ({ ok: true }) },
): LifecycleService {
...
describe('LifecycleService', () => {
  ...
  async function initRepo(): Promise<string> {
    const repoRoot = await mkdtemp(join(tmpdir(), 'taskflow-lifecycle-'))
  ...
  it('creates a managed host worktree with metadata, env files, and tmux layout', async () => {
    ...
    const gitDir = new NodeGitGateway().resolveWorktreeGitDir(worktreePath)
    ...
    expect(runtimeEnvText).toContain('PORTTA_FLOW_WORKTREE_PATH=')
    expect(runtimeEnvText).toContain('CUSTOM_TOKEN=abc123')
    expect(controlEnvText).toContain('PORTTA_FLOW_CONTROL_URL=http://127.0.0.1:5111/api/runtime/events')
    expect(hooks.calls).toEqual([
      expect.objectContaining({
        ...
        env: expect.objectContaining({
          ...
          FEATURE_FLAG: 'true',
          FRONTEND_PORT: '3010',
          PORTTA_FLOW_BRANCH: 'feature/search',
  ...
  it('creates only the agent and runtime panes for an environment-provider session', async () => {
    const repoRoot = await initRepo()
    const runtime = new ProjectRuntime()
    const tmux = new FakeSessionGateway()
    const config: ProjectConfig = {
      ...
      profiles: {
        default: {
          runtime: 'host',
          environment: { provider: 'compose' },
          envPassthrough: [],
          panes: [{ id: 'agent', kind: 'agent', focus: true }],
        },
      },
    }
    const lifecycle = makeLifecycleService(repoRoot, tmux, runtime, undefined, undefined, config)

    await lifecycle.createWorktree({ branch: 'feature/environment-shell' })

    expect(await tmux.listWindows()).toContainEqual(
      expect.objectContaining({
        windowName: buildWorktreeWindowName('feature/environment-shell'),
        paneCount: 2,
      }),
    )
    const runtimePaneCommand = tmux.createdPanes.find((entry) => entry.command?.includes(' monitor'))?.command
    expect(runtimePaneCommand).toContain('portta flow environment "$PORTTA_FLOW_BRANCH" monitor')
    expect(runtimePaneCommand).toContain('portta/runtime.env')
    expect(runtimePaneCommand).toContain('PORTTA_FLOW_CONTROL_URL=')
    expect(runtimePaneCommand).toContain('http://127.0.0.1:5111/api/runtime/events')
    expect(tmux.commands.some((entry) => entry.command.includes(' monitor'))).toBe(false)
  })

  it('launches an environment-provider Runtime pane inside its Docker worktree', async () => {
    const repoRoot = await initRepo()
    const runtime = new ProjectRuntime()
    const tmux = new FakeSessionGateway()
    ...
    const lifecycle = makeLifecycleService(repoRoot, tmux, runtime, docker, undefined, config)

    await lifecycle.createWorktree({ branch: 'feature/docker-environment', profile: 'sandbox' })

    const runtimePaneCommand = tmux.createdPanes.find((entry) => entry.command?.includes(' monitor'))?.command
    expect(runtimePaneCommand).toContain('docker exec -it')
    expect(runtimePaneCommand).toContain('taskflow-feature/docker-environment-container')
    expect(runtimePaneCommand).toContain('__worktrees/feature/docker-environment')
    expect(runtimePaneCommand).toContain('portta/runtime.env')
    expect(runtimePaneCommand).toContain('http://host.docker.internal:5111/api/runtime/events')
    expect(tmux.commands.some((entry) => entry.command.includes(' monitor'))).toBe(false)
  })

  it('creates only the Runtime pane for a Web Chat session', async () => {
    const repoRoot = await initRepo()
    const runtime = new ProjectRuntime()
    const tmux = new FakeSessionGateway()
    const config: ProjectConfig = {
    ...
    await lifecycle.createWorktree({ branch: 'feature/web-chat', interfaceMode: 'web_chat' })

    expect(await tmux.listWindows()).toContainEqual(
      expect.objectContaining({
        windowName: buildWorktreeWindowName('feature/web-chat'),
        paneCount: 1,
      }),
    )
    expect(tmux.createdWindows[0]?.command).toContain('portta flow environment "$PORTTA_FLOW_BRANCH" monitor')
    expect(tmux.createdWindows[0]?.command).toContain('portta/runtime.env')
    expect(tmux.commands).toEqual([])
    const worktreePath = join(repoRoot, '__worktrees', 'feature', 'web-chat')
    const gitDir = new NodeGitGateway().resolveWorktreeGitDir(worktreePath)
    expect((await readWorktreeMeta(gitDir))?.interfaceMode).toBe('web_chat')
  })

  it('uses the configured host Runtime command and working directory', async () => {
    const repoRoot = await initRepo()
    const runtime = new ProjectRuntime()
    const tmux = new FakeSessionGateway()
    const config: ProjectConfig = {
      ...
      profiles: {
        default: {
          ...
          panes: [
            { id: 'agent', kind: 'agent', focus: true },
            { id: 'runtime', kind: 'runtime', split: 'right' },
          ],
        },
      },
    }
    ...
    await lifecycle.createWorktree({ branch: 'feature/host-runtime' })

    const runtimePaneCommand = tmux.createdPanes.at(-1)?.command
    expect(runtimePaneCommand).toContain('cd --')
    expect(runtimePaneCommand).toContain('apps/web')
    expect(runtimePaneCommand).toContain('npm run dev')
    expect(runtimePaneCommand).toContain('npm run dev:stop')
    expect(tmux.commands.some((entry) => entry.command.includes('npm run dev'))).toBe(false)
  })
  ...
  it('writes no control.env when no control base URL is configured', async () => {
    ...
    expect(await nodeTest.file(paths.controlEnvPath).exists()).toBe(false)
    // The runtime env (unrelated to control reporting) is still written.
    expect(await nodeTest.file(paths.runtimeEnvPath).exists()).toBe(true)
  })
  ...
  it('rolls back the first paired worktree when the second branch cannot be created', async () => {
    ...
    expect(
      await tmux.hasWindow(buildProjectSessionName(repoRoot), buildWorktreeWindowName('claude-feature/search')),
    ).toBe(false)
  })
  ...
  it('refreshes runtime env after postCreate so system prompts see .env.local values', async () => {
    ...
    const hooks = new FakeHookRunner(async (input) => {
      await nodeTest.write(join(input.cwd, '.env.local'), `DATABASE_URL=${databaseUrl}\n`)
    })
    const lifecycle = makeLifecycleService(repoRoot, tmux, runtime, new FakeDockerGateway(), hooks, {
      ...TEST_CONFIG,
      profiles: {
    ...
    const gitDir = new NodeGitGateway().resolveWorktreeGitDir(worktreePath)
    const runtimeEnvText = await nodeTest.file(getWorktreeStoragePaths(gitDir).runtimeEnvPath).text()
    const agentCommand = tmux.commands.find(
      ({ target }) =>
  ...
  it('appends the oneshot system prompt to fresh launches when source is oneshot', async () => {
    ...
    const runtime = new ProjectRuntime()
    const tmux = new FakeSessionGateway()
    const lifecycle = makeLifecycleService(repoRoot, tmux, runtime, new FakeDockerGateway(), new FakeHookRunner(), {
      ...TEST_CONFIG,
      profiles: {
        ...TEST_CONFIG.profiles,
        default: {
  ...
  it('launches unattended Codex oneshots with automatic approval review', async () => {
    const repoRoot = await initRepo()
    const runtime = new ProjectRuntime()
    const tmux = new FakeSessionGateway()
    const lifecycle = makeLifecycleService(repoRoot, tmux, runtime, new FakeDockerGateway(), new FakeHookRunner(), {
    ...
    await lifecycle.createWorktree({
      branch: 'feature/codex-oneshot-with-approvals',
      source: 'oneshot',
    })

    expect(tmux.commands.at(-1)?.command).toContain('--approve-for-me')
    expect(tmux.commands.at(-1)?.command).not.toContain('--yolo')
  })

  it('rejects unsupported Codex versions before creating an oneshot worktree', async () => {
    const repoRoot = await initRepo()
    const runtime = new ProjectRuntime()
    const tmux = new FakeSessionGateway()
    const lifecycle = makeLifecycleService(
      ...
      { listSessionIds: async () => [] },
      'http://127.0.0.1:5111',
      { checkCodexAutoReview: async () => ({ ok: false, reason: '--approve-for-me is unavailable' }) },
    )

    await expect(lifecycle.createWorktree({ branch: 'feature/unsupported-codex', source: 'oneshot' })).rejects.toThrow(
      'Update Codex or use an explicit yolo profile',
    )

    expect(new NodeGitGateway().listWorktrees(repoRoot)).toHaveLength(1)
    expect(run(['git', 'branch', '--list', 'feature/unsupported-codex'], repoRoot)).toBe('')
  })

  it('arms an existing non-yolo Codex worktree with automatic approval review', async () => {
    const repoRoot = await initRepo()
    const runtime = new ProjectRuntime()
    const tmux = new FakeSessionGateway()
    const lifecycle = makeLifecycleService(repoRoot, tmux, runtime, new FakeDockerGateway(), new FakeHookRunner(), {
    ...
    await lifecycle.createWorktree({ branch: 'feature/existing-codex' })
    await lifecycle.closeWorktree('feature/existing-codex')

    await lifecycle.openWorktree('feature/existing-codex', { oneshot: { autoCloseOnDone: true } })

    const worktreePath = join(repoRoot, '__worktrees', 'feature', 'existing-codex')
    const gitDir = new NodeGitGateway().resolveWorktreeGitDir(worktreePath)
    expect((await readWorktreeMeta(gitDir))?.oneshot).toEqual({ autoCloseOnDone: true })
    expect(tmux.commands.at(-1)?.command).toContain('--approve-for-me')
  })
  ...
  it('launches custom agents through interpolated command templates', async () => {
    ...
    const lifecycle = makeLifecycleService(repoRoot, tmux, runtime, new FakeDockerGateway(), new FakeHookRunner(), {
      ...
      agents: {
        gemini: {
          label: 'Gemini CLI',
          startCommand:
            'gemini --task "${PROMPT}" --cwd "${WORKTREE_PATH}" --repo "${REPO_PATH}" --branch "${BRANCH}" --profile "${PROFILE}"',
    ...
    const meta = await readWorktreeMeta(
      new NodeGitGateway().resolveWorktreeGitDir(join(repoRoot, '__worktrees', 'feature', 'custom-agent')),
    )

    expect(meta?.agent).toBe('gemini')
    expect(agentCommand).toContain("export PORTTA_FLOW_AGENT_PROMPT='fix the search flow'")
    expect(agentCommand).toContain(`export PORTTA_FLOW_AGENT_REPO_PATH='${repoRoot}'`)
    expect(agentCommand).toContain(
      'gemini --task "$PORTTA_FLOW_AGENT_PROMPT" --cwd "$PORTTA_FLOW_AGENT_WORKTREE_PATH" --repo "$PORTTA_FLOW_AGENT_REPO_PATH" --branch "$PORTTA_FLOW_AGENT_BRANCH" --profile "$PORTTA_FLOW_AGENT_PROFILE"',
    )
    ...
    expect(reopenCommand).toContain('gemini resume --branch "$PORTTA_FLOW_AGENT_BRANCH"')
  })
  ...
  it('reopens custom agents without resume support in fresh mode', async () => {
    ...
    const reopenCommand = tmux.commands.at(-1)?.command
    expect(reopenCommand).toContain(
      'gemini --system "$PORTTA_FLOW_AGENT_SYSTEM_PROMPT" --task "$PORTTA_FLOW_AGENT_PROMPT" --branch "$PORTTA_FLOW_AGENT_BRANCH"',
    )
    expect(reopenCommand).toContain("export PORTTA_FLOW_AGENT_SYSTEM_PROMPT='Database: ")
    expect(reopenCommand).not.toContain("export PORTTA_FLOW_AGENT_SYSTEM_PROMPT=''")
  ...
  it('reinstalls Claude runtime hooks after postCreate rewrites settings.local.json', async () => {
    ...
    const settingsText = await nodeTest
      .file(join(repoRoot, '__worktrees', 'feature', 'hook-settings', '.claude', 'settings.local.json'))
      .text()

    expect(settingsText).toContain(additionalDirectory)
    expect(settingsText).toContain('portta-agentctl')
  ...
  it('creates a managed worktree under an absolute worktree root', async () => {
    const repoRoot = await initRepo()
    const absoluteWorktreeRoot = await mkdtemp(join(tmpdir(), 'taskflow-absolute-worktrees-'))
    ...
    const worktrees = new NodeGitGateway().listWorktrees(repoRoot)

    expect(
      worktrees.some((entry) => entry.branch === 'feature/absolute-root' && sameFsPath(entry.path, worktreePath)),
    ).toBe(true)
    expect(await nodeTest.file(join(worktreePath, 'README.md')).exists()).toBe(true)
  })

  it('keeps an internal Taskflow worktree root out of project status via git local excludes', async () => {
    const repoRoot = await initRepo()
    const lifecycle = makeLifecycleService(repoRoot, new FakeSessionGateway(), new ProjectRuntime())

    await lifecycle.createWorktree({ branch: 'feature/internal-root' })

    const gitDir = new NodeGitGateway().resolveWorktreeGitDir(repoRoot)
    const excludes = await readFile(join(gitDir, 'info', 'exclude'), 'utf8')
    expect(excludes).toContain('/__worktrees/')
    expect(run(['git', 'status', '--short'], repoRoot)).toBe('')
  })
  ...
  it('creates a managed worktree for an existing local branch', async () => {
    ...
    expect(
      new NodeGitGateway()
        ...
        .some((entry) => entry.branch === 'feature-follow' && sameFsPath(entry.path, worktreePath)),
    ).toBe(true)
  ...
  it('lists available branches excluding branches already checked out', async () => {
    ...
    expect(lifecycle.listAvailableBranches()).toEqual([{ name: 'feature-available' }])
  })

  it('lists local branches by default and includes remote branches when requested', async () => {
    const repoRoot = await initRepo()
    const remoteRoot = await mkdtemp(join(tmpdir(), 'taskflow-lifecycle-remote-'))
    const cloneRoot = await mkdtemp(join(tmpdir(), 'taskflow-lifecycle-clone-'))
    ...
    await nodeTest.write(join(cloneRoot, 'remote.txt'), 'remote branch\n')
    ...
    expect(lifecycle.listAvailableBranches()).toEqual([{ name: 'feature-local-only' }])
  ...
  it('creates a managed worktree from an existing remote-only branch', async () => {
    const repoRoot = await initRepo()
    const remoteRoot = await mkdtemp(join(tmpdir(), 'taskflow-lifecycle-remote-existing-'))
    const cloneRoot = await mkdtemp(join(tmpdir(), 'taskflow-lifecycle-clone-existing-'))
    tempDirs.push(remoteRoot, cloneRoot)
    ...
    expect(
      new NodeGitGateway()
        .listWorktrees(repoRoot)
        .some((entry) => entry.branch === 'feature-remote-existing' && sameFsPath(entry.path, worktreePath)),
    ).toBe(true)
    expect(run(['git', 'branch', '--show-current'], worktreePath)).toBe('feature-remote-existing')
    expect(run(['git', 'rev-parse', '--abbrev-ref', '--symbolic-full-name', '@{upstream}'], worktreePath)).toBe(
      'origin/feature-remote-existing',
    )
  })

  it('removes the temporary local branch when remote-only worktree creation fails', async () => {
    const repoRoot = await initRepo()
    const remoteRoot = await mkdtemp(join(tmpdir(), 'taskflow-lifecycle-remote-rollback-'))
    const cloneRoot = await mkdtemp(join(tmpdir(), 'taskflow-lifecycle-clone-rollback-'))
    tempDirs.push(remoteRoot, cloneRoot)
  ...
  it('keeps an existing branch when creation fails after the worktree is created', async () => {
    ...
    expect(run(['git', 'branch', '--list', 'feature-existing-rollback'], repoRoot)).toContain(
      'feature-existing-rollback',
    )
  })
  ...
  it('creates a managed worktree from an explicit base branch', async () => {
    ...
    await nodeTest.write(join(repoRoot, 'README.md'), '# release base\n')
    run(['git', 'add', 'README.md'], repoRoot)
    ...
    expect(await nodeTest.file(join(worktreePath, 'README.md')).text()).toBe('# release base\n')
  })
  ...
  it('reopens a managed codex worktree with codex resume --last', async () => {
    ...
    expect(agentCommand).toContain('codex --enable hooks')
    expect(agentCommand).toContain('codex-notify')
    expect(agentCommand).toContain('--yolo resume --last')
  ...
  it('reopens a stale managed codex terminal from its saved app-server thread', async () => {
    ...
    expect(agentCommand).toContain('codex --enable hooks')
    expect(agentCommand).toContain('codex-notify')
    expect(agentCommand).toContain("--yolo resume 'thread-open-refresh'")
  ...
  it('refreshes a managed codex terminal from its saved app-server thread', async () => {
    ...
    expect(agentCommand).toContain('codex --enable hooks')
    expect(agentCommand).toContain('codex-notify')
    expect(agentCommand).toContain("--yolo resume 'thread-refresh'")
    expect(agentCommand).not.toContain('resume --last')
  ...
  it('refreshes a managed codex terminal from its discovered CLI session', async () => {
    const repoRoot = await initRepo()
    const runtime = new ProjectRuntime()
    const tmux = new FakeSessionGateway()
    const lifecycle = makeLifecycleService(
      ...
      new FakeAutoNameService(),
      {},
      { listSessionIds: async () => ['thread-from-rollout'] },
    )

    await lifecycle.createWorktree({ branch: 'feature-codex-cli-refresh' })
    tmux.commands.length = 0

    await lifecycle.refreshAgentTerminal('feature-codex-cli-refresh')

    expect(tmux.commands.at(-1)?.command).toContain('codex --enable hooks')
    expect(tmux.commands.at(-1)?.command).toContain('codex-notify')
    expect(tmux.commands.at(-1)?.command).toContain("resume 'thread-from-rollout'")
    const worktreePath = join(repoRoot, '__worktrees', 'feature-codex-cli-refresh')
    const gitDir = new NodeGitGateway().resolveWorktreeGitDir(worktreePath)
    expect((await readWorktreeMeta(gitDir))?.tabs?.find((tab) => tab.kind === 'root')?.sessionId).toBe(
      'thread-from-rollout',
    )
  })
  ...
  it('closes the tmux window without removing the worktree or branch', async () => {
    ...
    expect(await tmux.listWindows()).toEqual([])
    expect(new NodeGitGateway().listWorktrees(repoRoot).some((entry) => entry.branch === 'feature-close')).toBe(true)
  ...
  it('closes a worktree before archiving it', async () => {
    ...
    expect(sameFsPath(archiveState.entries[0]?.path ?? '', join(repoRoot, '__worktrees', 'feature-archive'))).toBe(true)
  })
  ...
  it('updates and clears a worktree label in metadata and runtime state', async () => {
    ...
    const gitDir = new NodeGitGateway().resolveWorktreeGitDir(worktreePath)
    const paths = getWorktreeStoragePaths(gitDir)
    await nodeTest.write(paths.runtimeEnvPath, 'runtime-marker\n')
    await nodeTest.write(paths.controlEnvPath, 'control-marker\n')
    ...
    expect(await nodeTest.file(paths.runtimeEnvPath).text()).toBe('runtime-marker\n')
    expect(await nodeTest.file(paths.controlEnvPath).text()).toBe('control-marker\n')
  ...
  it('rejects labeling unmanaged worktrees without creating metadata', async () => {
    ...
    await expect(lifecycle.setWorktreeLabel('feature-unmanaged-label', 'Search ranking')).rejects.toMatchObject({
      status: 409,
      message: 'Worktree feature-unmanaged-label has no managed metadata to label',
    })
    expect(await nodeTest.file(paths.metaPath).exists()).toBe(false)
    expect(await nodeTest.file(paths.runtimeEnvPath).exists()).toBe(false)
    expect(await nodeTest.file(paths.controlEnvPath).exists()).toBe(false)
  })
  ...
  it('tears down the container when a worktree leaves its docker profile', async () => {
    ...
    const controlEnvText = await nodeTest.file(getWorktreeStoragePaths(gitDir).controlEnvPath).text()
    expect(controlEnvText).toContain('PORTTA_FLOW_CONTROL_URL=http://127.0.0.1:5111/api/runtime/events')
  })
  ...
  it('rejects switching to an unknown profile', async () => {
    ...
    await expect(lifecycle.setWorktreeProfile('feature-profile-unknown', 'nope')).rejects.toMatchObject({
      status: 400,
  ...
  it('creates a managed docker worktree through the container runtime path', async () => {
    ...
    expect(docker.launched[0]?.runtimeEnv.PORTTA_FLOW_RUNTIME).toBe('docker')
    ...
    expect(controlEnvText).toContain('PORTTA_FLOW_CONTROL_URL=http://host.docker.internal:5111/api/runtime/events')
  ...
  it('starts one-pane docker agent sessions without nesting docker exec inside the container shell', async () => {
    ...
    expect(windowCommand).toContain('taskflow-feature-sandbox-agent-container')
    expect(agentCommand).toContain('claude')
  ...
  it('refreshes docker control env with a host-reachable callback when reopening', async () => {
    ...
    const gitDir = new NodeGitGateway().resolveWorktreeGitDir(worktreePath)
    const paths = getWorktreeStoragePaths(gitDir)
    const staleControlEnvText = (await nodeTest.file(paths.controlEnvPath).text()).replace(
      'http://host.docker.internal:5111/api/runtime/events',
      'http://127.0.0.1:5111/api/runtime/events',
    )
    await nodeTest.write(paths.controlEnvPath, staleControlEnvText)

    await lifecycle.closeWorktree('feature-sandbox-reopen')
    await lifecycle.openWorktree('feature-sandbox-reopen')

    const refreshedControlEnvText = await nodeTest.file(paths.controlEnvPath).text()

    expect(refreshedControlEnvText).toContain(
      'PORTTA_FLOW_CONTROL_URL=http://host.docker.internal:5111/api/runtime/events',
    )
  })
  ...
  it('clears creation progress when the first phase callback fails', async () => {
    ...
    await expect(lifecycle.createWorktree({ branch: 'feature/progress-failure' })).rejects.toThrow('progress failed')
  ...
  it('refuses to remove a worktree with uncommitted changes, and discards them when forced', async () => {
    const repoRoot = await initRepo()
    const runtime = new ProjectRuntime()
    const tmux = new FakeSessionGateway()
    const hooks = new FakeHookRunner()
    ...
    await nodeTest.write(join(worktreePath, 'README.md'), '# dirty\n')

    // The branch is deleted with --force, so an unguarded removal destroys them.
    await expect(lifecycle.removeWorktree('feature-dirty')).rejects.toThrow(/uncommitted changes/)
    expect(hooks.calls.filter((call) => call.name === 'preRemove')).toHaveLength(0)
    expect(new NodeGitGateway().listWorktrees(repoRoot).some((entry) => sameFsPath(entry.path, worktreePath))).toBe(
      true,
    )

    await lifecycle.removeWorktree('feature-dirty', { force: true })
  ...
  it('refuses to remove a worktree holding commits that exist nowhere else', async () => {
    const repoRoot = await initRepo()
    const runtime = new ProjectRuntime()
    const tmux = new FakeSessionGateway()
    const hooks = new FakeHookRunner()
    const git = new AheadTrackingGitGateway(new Set(['feature-ahead']))
    const lifecycle = makeLifecycleService(repoRoot, tmux, runtime, new FakeDockerGateway(), hooks, TEST_CONFIG, git)

    await lifecycle.createWorktree({ branch: 'feature-ahead' })

    await expect(lifecycle.removeWorktree('feature-ahead')).rejects.toThrow(/only here/)
    expect(run(['git', 'branch', '--list', 'feature-ahead'], repoRoot)).not.toBe('')

    await lifecycle.removeWorktree('feature-ahead', { force: true })
  ...
  it('prunes only closed worktrees, leaving open ones untouched', async () => {
    ...
    expect(hooks.calls.filter((call) => call.name === 'preRemove').map((call) => canonicalizeFsPath(call.cwd))).toEqual(
      [canonicalizeFsPath(join(repoRoot, '__worktrees', 'feature-prune-host'))],
    )
    expect(docker.removed).toEqual([])
    expect(
      new NodeGitGateway()
        .listWorktrees(repoRoot)
        .filter((entry) => !sameFsPath(entry.path, repoRoot))
        .map((entry) => entry.branch),
    ).toEqual(['feature-prune-docker'])
  ...
  it('keeps a worktree whose work exists nowhere else out of a prune', async () => {
    const repoRoot = await initRepo()
    const runtime = new ProjectRuntime()
    const tmux = new FakeSessionGateway()
    const hooks = new FakeHookRunner()
    ...
    await lifecycle.createWorktree({ branch: 'feature-prune-clean' })
    await lifecycle.createWorktree({ branch: 'feature-prune-dirty' })
    await nodeTest.write(join(repoRoot, '__worktrees', 'feature-prune-dirty', 'README.md'), '# dirty\n')
    await lifecycle.closeWorktree('feature-prune-clean')
    await lifecycle.closeWorktree('feature-prune-dirty')

    // A sweep is not a per-worktree decision, so it never discards anything.
    const result = await lifecycle.pruneWorktrees()

    expect(result.removedBranches).toEqual(['feature-prune-clean'])
    expect(result.keptBranches).toEqual([{ branch: 'feature-prune-dirty', reason: 'uncommitted changes' }])
    expect(run(['git', 'branch', '--list', 'feature-prune-dirty'], repoRoot)).not.toBe('')
  }, 15_000)
  ...
  it('removes the sandbox container before deleting a docker worktree', async () => {
    ...
    expect(
      hooks.calls.some(
        (call) =>
          call.name === 'preRemove' &&
          sameFsPath(call.cwd, join(repoRoot, '__worktrees', 'feature-remove-docker')) &&
          call.env.PORTTA_FLOW_RUNTIME === 'docker',
      ),
    ).toBe(true)
    expect(docker.removed).toContain('feature-remove-docker')
    expect(new NodeGitGateway().listWorktrees(repoRoot).some((entry) => entry.branch === 'feature-remove-docker')).toBe(
      false,
    )
  })
  ...
  it('falls back to the first configured profile when no default profile exists', async () => {
    ...
    const createdGitDir = new NodeGitGateway().resolveWorktreeGitDir(
    ...
    new NodeGitGateway().createWorktree({
      repoRoot,
    ...
    const openedGitDir = new NodeGitGateway().resolveWorktreeGitDir(unmanagedPath)
  ...
  it('merges a clean worktree into main and removes it on success', async () => {
    ...
    await nodeTest.write(join(worktreePath, 'README.md'), '# merged change\n')
    ...
    expect(await nodeTest.file(join(repoRoot, 'README.md')).text()).toContain('merged change')
  })
  ...
  it('merges and cleans up a worktree even when the source branch is ahead', async () => {
    ...
    await nodeTest.write(join(worktreePath, 'README.md'), '# merged ahead change\n')
    run(['git', 'add', 'README.md'], worktreePath)
    run(['git', 'commit', '-m', 'feature ahead change'], worktreePath)

    await lifecycle.mergeWorktree('feature-merge-ahead')

    expect(hooks.calls.some((call) => call.name === 'preRemove' && sameFsPath(call.cwd, worktreePath))).toBe(true)
    expect(new NodeGitGateway().listWorktrees(repoRoot).some((entry) => sameFsPath(entry.path, worktreePath))).toBe(
      false,
    )
    expect(run(['git', 'branch', '--list', 'feature-merge-ahead'], repoRoot)).toBe('')
    expect(await nodeTest.file(join(repoRoot, 'README.md')).text()).toContain('merged ahead change')
  })
  ...
  it('forks a tab into a parked pane and swaps it into the visible agent slot', async () => {
    ...
    const gitDir = new NodeGitGateway().resolveWorktreeGitDir(join(repoRoot, '__worktrees', 'feature-fork'))
    const meta = await readWorktreeMeta(gitDir)
  ...
  it('selects another tab by swapping its parked pane back into the visible slot', async () => {
    ...
    const gitDir = new NodeGitGateway().resolveWorktreeGitDir(join(repoRoot, '__worktrees', 'feature-select'))
  ...
  it('deletes a fork tab, swapping the root back on-screen and killing the fork pane', async () => {
    ...
    const gitDir = new NodeGitGateway().resolveWorktreeGitDir(join(repoRoot, '__worktrees', 'feature-del'))
  ...
  it('rebuilds parked fork panes and re-activates the previous tab on reopen', async () => {
    ...
    const gitDir = new NodeGitGateway().resolveWorktreeGitDir(join(repoRoot, '__worktrees', 'feature-restore'))
    const meta = await readWorktreeMeta(gitDir)
    expect(meta?.activeTabId).toBe('fork-1')
  ...
  it('rebuilds parked fork panes when the codex agent terminal is refreshed', async () => {
    ...
    const gitDir = new NodeGitGateway().resolveWorktreeGitDir(
