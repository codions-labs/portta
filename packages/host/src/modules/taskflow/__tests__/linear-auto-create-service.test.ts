...
import type { CreateLifecycleWorktreeInput } from '../services/lifecycle-service.ts'
import {
...
} from '../services/linear-auto-create-service.ts'
import type { FetchIssuesResult, LinearIssue } from '../services/linear-service.ts'
...
describe('filterAutoCreateIssues', () => {
  ...
  it('keeps Todo issues with the taskflow label that do not already have a worktree', () => {
...
describe('watchTeamKeys filter', () => {
  ...
  it('drops issues whose team key is not in the (uppercase, normalized) allowlist', () => {
    ...
    const ops = createIssue({ id: 'ops', identifier: 'OPS-1', branchName: 'ops-1', team: { name: 'Ops', key: 'OPS' } })
    const design = createIssue({
      id: 'des',
      identifier: 'DES-1',
      branchName: 'des-1',
      team: { name: 'Design', key: 'DES' },
    })
    expect(filterAutoCreateIssues([eng, ops, design], [], ['ENG', 'OPS']).map((i) => i.identifier)).toEqual([
      'ENG-1',
      'OPS-1',
    ])
  })
  ...
  it('applies the same filter to the oneshot variant', () => {
    const ops = createIssue({
      ...
      identifier: 'OPS-1',
      branchName: 'ops-1',
      labels: [{ name: 'taskflow_oneshot', color: '#fff' }],
...
describe('filterAutoOneshotIssues', () => {
  ...
  it('matches the taskflow_oneshot label case-insensitively', () => {
    const issues = [
      createIssue({
        id: 'a',
        identifier: 'ENG-1',
      ...
      createIssue({ id: 'c', identifier: 'ENG-3', branchName: 'eng-3', labels: [{ name: 'taskflow', color: '#fff' }] }),
    ]
  ...
  it('routes issues tagged with both taskflow and taskflow_oneshot to the oneshot filter only', () => {
    const issues = [
      createIssue({
        id: 'a',
        identifier: 'ENG-1',
        branchName: 'eng-1',
        labels: [
          { name: 'taskflow', color: '#fff' },
          { name: 'taskflow_oneshot', color: '#fff' },
        ],
      }),
    ]
...
describe('runLinearAutoCreateOnce', () => {
  ...
  it("dedupes permanent oneshot failures so they don't retry every poll", async () => {
    const issue = createIssue({ labels: [{ name: 'taskflow_oneshot', color: '#fff' }] })
    let oneshotCalls = 0
    const deps: LinearAutoCreateDependencies = {
      lifecycleService: {
        async createWorktree(): Promise<{ branch: string; worktreeId: string }> {
  ...
  it('calls runOneshotForIssue for taskflow_oneshot-labeled issues and skips taskflow create for them', async () => {
    const oneshotIssue = createIssue({
      id: 'issue-oneshot',
      identifier: 'ENG-200',
      branchName: 'eng-200-oneshot',
      labels: [{ name: 'taskflow_oneshot', color: '#fff' }],
    })
    const createIssue1 = createIssue({
      id: 'issue-create',
      identifier: 'ENG-201',
      branchName: 'eng-201-create',
      labels: [{ name: 'taskflow', color: '#fff' }],
    })
  ...
  it('does not notify onOneshotPickedUp for the regular `taskflow` (create) path', async () => {
  ...
  it("notifies onOneshotPickedUp with the branch resolved by runOneshotForIssue, not the issue's branchName", async () => {
    ...
    expect(pickups).toEqual([{ identifier: 'ENG-200', branch: 'eng-200-resumed-from-attachment' }])
  })
  ...
  it('skips taskflow_oneshot issues when no runOneshotForIssue dep is provided', async () => {
    const oneshotIssue = createIssue({
      id: 'issue-oneshot',
      identifier: 'ENG-200',
      branchName: 'eng-200-oneshot',
