...
const originalLinearApiKey = nodeTest.env.LINEAR_API_KEY
...
describe('Linear issue creation', () => {
  beforeEach(() => {
    resetLinearCaches()
    nodeTest.env.LINEAR_API_KEY = 'test-linear-key'
  ...
  afterEach(() => {
    ...
    if (originalLinearApiKey === undefined) {
      delete nodeTest.env.LINEAR_API_KEY
      delete process.env.LINEAR_API_KEY
      return
    }
    nodeTest.env.LINEAR_API_KEY = originalLinearApiKey
  ...
  it('invalidates the assigned issue cache after creating a ticket', async () => {
    ...
    globalThis.fetch = async function fetchMock(_input: RequestInfo | URL, init?: RequestInit): Promise<Response> {
...
function attachment(
  partial: Partial<LinearAttachment> & { id: string; url: string; title: string; createdAt: string },
): LinearAttachment {
  return {
    subtitle: null,
    sourceType: null,
...
describe('findTaskflowAttachment', () => {
  it('returns null when no taskflow attachments are present', () => {
    expect(findTaskflowAttachment({ attachments: [] })).toBeNull()
    expect(
      findTaskflowAttachment({
        attachments: [attachment({ id: 'a', url: 'u', title: 'some PR', createdAt: '2026-01-01' })],
      }),
    ).toBeNull()
  })

  it('prefers an exact branch match over the newest', () => {
    const result = findTaskflowAttachment(
      {
        attachments: [
          attachment({ id: '1', url: 'u1', title: 'taskflow-state:feat/other', createdAt: '2026-05-01' }),
          attachment({ id: '2', url: 'u2', title: 'taskflow-state:feat/foo', createdAt: '2026-04-01' }),
        ],
      },
      'feat/foo',
    )
  ...
  it('falls back to newest taskflow attachment when no branch hint matches', () => {
    const result = findTaskflowAttachment({
      attachments: [
        attachment({ id: '1', url: 'u1', title: 'taskflow-state:feat/other', createdAt: '2026-04-01' }),
        attachment({ id: '2', url: 'u2', title: 'taskflow-state:feat/old', createdAt: '2026-05-01' }),
      ],
    })
    expect(result?.id).toBe('2')
  })
  ...
  it("ignores a linked github issue tagged sourceType 'github' (not a PR)", () => {
    ...
    const result = findLinkedGitHubPr({
      attachments: [
        attachment({
          id: '1',
          url: 'https://github.com/org/repo/issues/10105',
          title: '#10105 bug',
          createdAt: '2026-05-01',
          sourceType: 'github',
        }),
...
describe('buildLinearPickupMarkdown', () => {
  it('renders the pickup comment verbatim', () => {
    ...
    expect(md).toBe('**Taskflow pickup — branch `eng-200-oneshot`**\n' + '\n' + '- Picked up: 2026-05-20T12:34:56.789Z')
  })
})
...
describe('buildLinearSummaryMarkdown', () => {
  it('includes turns and the attachment title', () => {
    ...
    // (--from-linear is still valid on `taskflow add`, so don't assert against it.)
    expect(md).toContain('portta flow oneshot --linear')
