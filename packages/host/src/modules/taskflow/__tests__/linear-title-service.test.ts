...
import type { LinearIssue, SearchTeamIssuesResult } from '../services/linear-service.ts'
import {
  extractKeywords,
  findDuplicateLinearIssue,
  polishLinearIssueTitle,
  resolveLinearTicketTitle,
} from '../services/linear-title-service.ts'
import type { RunLlmResult } from '../services/llm-spawn.ts'
...
describe('extractKeywords', () => {
  it('drops stopwords and short tokens', () => {
    expect(extractKeywords('Fix the login redirect on token refresh')).toEqual(['fix', 'login', 'redirect', 'token'])
  })

  it('caps at the requested max', () => {
    expect(extractKeywords('alpha beta gamma delta epsilon zeta', 3)).toEqual(['alpha', 'beta', 'gamma'])
  })
...
describe('findDuplicateLinearIssue', () => {
  ...
  it('extracts identifier from chatty LLM output', async () => {
    const candidates = [makeIssue({ identifier: 'ENG-2', title: 'Logout drops session on refresh' })]
    for (const chatty of [
      'Answer: ENG-2',
