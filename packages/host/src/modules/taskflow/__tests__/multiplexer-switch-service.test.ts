...
import { type MultiplexerSwitchDependencies, switchMultiplexer } from '../services/multiplexer-switch-service.ts'
...
function makeDeps(overrides: Partial<MultiplexerSwitchDependencies> & { open?: string[] } = {}): Recorder {
  ...
  const deps: MultiplexerSwitchDependencies = {
    ...
    closeWorktree:
      overrides.closeWorktree ??
      (async (branch) => {
        log.push(`close:${branch}`)
      }),
    persistMultiplexer:
      overrides.persistMultiplexer ??
      (async (kind) => {
        log.push(`persist:${kind}`)
      }),
    openWorktree:
      overrides.openWorktree ??
      (async (branch) => {
        log.push(`open:${branch}`)
      }),
    ...(overrides.onProgress ? { onProgress: overrides.onProgress } : {}),
    ...(overrides.hasActiveRuns ? { hasActiveRuns: overrides.hasActiveRuns } : {}),
  }
...
describe('switchMultiplexer', () => {
  ...
  it('closes every open worktree, flips the config, then reopens them', async () => {
    ...
    expect(log).toEqual(['close:alpha', 'close:beta', 'persist:herdr', 'open:alpha', 'open:beta'])
  })
  ...
  it('aborts without touching the config when a close fails', async () => {
    ...
    if (!result.ok) {
      ...
      expect(result.failures).toEqual([{ branch: 'beta', stage: 'close', message: 'tmux window busy' }])
    }
  ...
  it('refuses to switch while a Taskflow Run is active', async () => {
    const { log, deps } = makeDeps({
      hasActiveRuns: () => true,
    })
    ...
    expect(result).toEqual({ ok: false, error: 'cannot switch multiplexer while a Run is active', failures: [] })
    expect(log).toEqual([])
  })
  ...
  it('emits progress for each stage', async () => {
    ...
    expect(seen).toEqual(['close:alpha', 'close:beta', 'persist', 'restore:alpha', 'restore:beta'])
  })
})
