import type { ManagedWorktreeRuntimeState } from 'portta-core/taskflow'
...
describe('buildNativeTerminalTmuxCommand', () => {
  it('uses isolated tmux socket and config when available', () => {
    expect(
      buildNativeTerminalTmuxCommand({
        PORTTA_FLOW_ISOLATED_TMUX_SOCKET: 'taskflow-native-demo',
        PORTTA_FLOW_ISOLATED_TMUX_CONFIG: '/tmp/taskflow.conf',
      }),
    ).toBe("tmux -L 'taskflow-native-demo' -f '/tmp/taskflow.conf'")
  })
  ...
  it('uses isolated tmux socket without config when available', () => {
    ...
    ).toBe("tmux -L 'taskflow-native-demo'")
  })
...
describe('buildNativeTerminalLaunch', () => {
  it('builds a grouped tmux attach command', () => {
    const launch = buildNativeTerminalLaunch({
      branch: 'feature/search',
      state: makeState(),
      tmuxCommand: "tmux -L 'taskflow-native-demo' -f '/tmp/taskflow.conf'",
      sessionPrefix: 'taskflow-native-5111-',
    })
    ...
    expect(launch.data.shellCommand).toContain('taskflow-native-5111-wt_feature_search')
    ...
    expect(launch.data.shellCommand).toContain('tf-project-12345678')
