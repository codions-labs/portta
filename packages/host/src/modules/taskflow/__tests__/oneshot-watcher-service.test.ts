import type { AgentLifecycle, ManagedWorktreeRuntimeState, WorktreeMeta, WorktreeSource } from 'portta-core/taskflow'
import { beforeEach, describe, expect, it, vi } from 'vitest'
import type { LifecycleService } from '../services/lifecycle-service.ts'
import { resetOneshotWatcherState, runOneshotWatch } from '../services/oneshot-watcher-service.ts'
...
function makeRuntime(worktrees: ManagedWorktreeRuntimeState[]): ProjectRuntime {
  ...
  return {
    ...
    getWorktreeByBranch: (branch: string) => worktrees.find((wt) => wt.branch === branch) ?? null,
...
function makeLifecycle(): {
  ...
  const service: OneshotLifecycleMock = {
    closeWorktree: vi.fn(async (branch: string) => {
      closeCalls.push(branch)
    }),
    disarmOneshot: vi.fn(async (branch: string) => {
...
describe('oneshot-watcher-service', () => {
  ...
  it('skips worktrees that are not oneshot source', async () => {
    ...
    const readMeta = vi.fn(async () => makeMeta({ autoCloseOnDone: true }))
  ...
  it('posts to Linear before closing when target is set', async () => {
    const lc = makeLifecycle()
    const calls: string[] = []
    lc.service.closeWorktree = vi.fn(async (branch: string) => {
      calls.push(`close:${branch}`)
    })
    const postToLinear = vi.fn(async (branch: string) => {
  ...
  it('respects autoCloseOnDone=false but still posts to Linear', async () => {
    const lc = makeLifecycle()
    const postToLinear = vi.fn(async () => {})
    await runOneshotWatch({
      projectRuntime: makeRuntime([makeWorktree({ branch: 'feat/a', lifecycle: 'stopped' })]),
      lifecycleService: lc.service,
      postToLinear,
  ...
  it('still closes + disarms when postToLinear fails', async () => {
    const lc = makeLifecycle()
    const postToLinear = vi.fn(async () => {
      throw new Error('linear API 502')
    })
    await runOneshotWatch({
      projectRuntime: makeRuntime([makeWorktree({ branch: 'feat/a', lifecycle: 'stopped' })]),
      lifecycleService: lc.service,
  ...
  it('still disarms when closeWorktree throws', async () => {
    const lc = makeLifecycle()
    lc.service.closeWorktree = vi.fn(async () => {
