...
describe('parsePrViewStatus — stale-entry refresh', () => {
  it('re-reads the draft flag of a PR that is still open', () => {
    ...
    expect(parsePrViewStatus(JSON.stringify({ state: 'OPEN', isDraft: false }))).toEqual({
      state: 'open',
      isDraft: false,
    })
    expect(parsePrViewStatus(JSON.stringify({ state: 'OPEN', isDraft: true }))).toEqual({
      state: 'open',
      isDraft: true,
    })
  })
  ...
  it('reports a merged PR as not draft', () => {
    expect(parsePrViewStatus(JSON.stringify({ state: 'MERGED', isDraft: false }))).toEqual({
      state: 'merged',
      isDraft: false,
    })
  })
