...
import { type ManagedProject, type ProjectLoopController, ProjectManager } from '../services/project-manager.ts'
...
describe('ProjectManager', () => {
  ...
  it('refuses to register a root rejected by assertAllowed', () => {
    ...
    const manager = new ProjectManager<FakeRuntime>({
      ...
      resolveRoot: (path) => path,
      assertAllowed: (root) => {
        if (root === '/not-allowed') throw new Error('outside allowlist')
      },
      createRuntime: ({ projectDir }) => ({ config: { name: `name:${projectDir}` } }),
    })

    expect(() => manager.add('/not-allowed')).toThrow('outside allowlist')
    expect(manager.list()).toEqual([])
    expect(registry.entries).toEqual([])
    expect(manager.add('/repo/alpha').entry.path).toBe('/repo/alpha')
  })
})
