import { OPEN_SESSIONS_STATE_VERSION, type OpenSessionsState } from 'portta-core/taskflow'
import { describe, expect, it } from 'vitest'
import type { GitWorktreeEntry } from '../adapters/git.ts'
import type { SessionWindowSummary } from '../adapters/session-gateway.ts'
...
} from '../services/session-restore-service.ts'
...
function window(windowName: string, sessionName = SESSION): SessionWindowSummary {
...
describe('saveOpenSessionsSnapshot', () => {
  function makeDeps(windows: SessionWindowSummary[]) {
    const writes: Array<{ gitDir: string; state: OpenSessionsState }> = []
    return {
      writes,
      deps: {
        git: {
          listLiveWorktrees: () => [worktree(PROJECT_DIR, 'main'), worktree('/repo/wt/feature-a', 'feature-a')],
          resolveWorktreeGitDir: (cwd: string) => `${cwd}/.git`,
        },
        sessions: { listWindows: async () => windows },
