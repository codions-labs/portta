import type { PrEntry } from 'portta-core/taskflow'
...
describe('buildProjectSnapshot', () => {
  it('projects runtime worktrees into frontend-facing snapshot state', () => {
    ...
    runtime.setServices('wt_search', [{ name: 'frontend', port: 3010, running: true, url: 'http://127.0.0.1:3010' }])
    ...
    expect(snapshot.worktrees).toEqual([
      {
        ...
        status: 'creating',
        elapsed: '5m',
        services: [{ name: 'frontend', port: 3010, running: true, url: 'http://127.0.0.1:3010' }],
        prs: [
          {
            repo: 'org/repo',
            number: 77,
  ...
  it('carries the issueRef stored on the worktree', () => {
    const runtime = new ProjectRuntime()
    runtime.upsertWorktree({
      worktreeId: 'wt_issue',
      branch: 'fix/proxy-timeout',
      path: '/repo/__worktrees/fix-proxy-timeout',
      runtime: 'host',
    ...
    expect(snapshot.worktrees[0]?.issueRef).toBe('github:acme/api#113')
  })
  ...
  it('orders open worktrees first, then closed ones by pr state', () => {
    ...
    const openSession = (worktreeId: string): void => {
      runtime.setSessionState(worktreeId, { exists: true, sessionName: `tf-${worktreeId}`, paneCount: 1 })
    }
