...
describe('terminal adapter', () => {
  ...
  it('replaces a prior browser attachment for the same worktree', async () => {
    ...
    cleanupStaleSessions()
    await attach('attach-a', { ownerSessionName: 'owner', windowName: 'tf-feature/search' }, 80, 24)
    await attach('attach-b', { ownerSessionName: 'owner', windowName: 'tf-feature/search' }, 80, 24)
    ...
    expect(new Set(afterAttach).size).toBe(1)
  ...
  it('sends ctrl-c to the target pane when interrupting a prompt', async () => {
    ...
    expect(tmuxCalls).toContainEqual(['tmux', 'send-keys', '-t', 'owner:tf-feature/search.0', 'C-c'])
  })
  ...
  it('waits before submitting a pasted prompt when a submit delay is provided', async () => {
    ...
    expect(tmuxCalls).toEqual([
      ['tmux', 'send-keys', '-t', 'owner:tf-feature/search.0', '-l', '--', 'Preamble:\n'],
      ['tmux', 'load-buffer', '-b', expect.stringMatching(/^taskflow-prompt-/), '-'],
      [
        'tmux',
        'paste-buffer',
        '-rp',
        '-b',
        expect.stringMatching(/^taskflow-prompt-/),
        '-t',
        'owner:tf-feature/search.0',
        '-d',
      ],
      ['tmux', 'send-keys', '-t', 'owner:tf-feature/search.0', 'Enter'],
    ])
  })
})
