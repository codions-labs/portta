import type { ProfileConfig, WorktreeMeta, WorktreeSnapshot } from 'portta-core/taskflow'
...
import { ok, type WorktreeConversationResult } from '../services/worktree-conversation-result.ts'
import {
...
describe('resolveCodexAppServerLaunchContext', () => {
  ...
  it('rejects Docker worktrees instead of changing execution context', () => {
    expect(
      resolveCodexAppServerLaunchContext({
        worktree: makeWorktree(),
  ...
  it('keeps non-yolo web chat sandboxed and routes approvals through automatic review', () => {
    expect(
      resolveCodexAppServerLaunchContext({
        worktree: makeWorktree(),
        meta: makeMeta(),
...
describe('WorktreeConversationService', () => {
  ...
  it('uses JSONL session messages as the Codex snapshot transcript when available', async () => {
    ...
    expect(result.data.conversation.messages.map((message) => message.id)).toEqual(['call-1', 'call-1:result'])
    expect(result.data.conversation.messages.map((message) => message.text)).toEqual(['bun test', 'pass'])
  })

  it('reads native Codex history without acquiring an app-server writer', async () => {
    const metaStore = new Map<string, WorktreeMeta>()
    const worktree = makeWorktree()
    const gitDir = `${worktree.path}/.git`
    metaStore.set(gitDir, makeMeta())
    ...
    const service = new WorktreeConversationService({
      ...
      resolveLaunchContext: () => ({
        ok: false,
        status: 409,
        error: 'interactive app-server access is unavailable',
      }),
      readSessionMessages: async () => [
        {
          id: 'assistant-live',
    ...
    expect(result.ok).toBe(true)
    if (!result.ok) return
    expect(result.data.conversation.messages[0]?.text).toBe('Native session reply')
    expect(appServer.calls).toEqual(['threadList'])
  })
  ...
  it('creates a new thread on attach when none can be resolved', async () => {
    ...
    expect(appServer.calls).toEqual(['threadList', 'threadStart:/tmp/worktrees/codex-feature'])
