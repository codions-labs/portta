...
export async function findClaudeSessionPath(sessionId: string, cwd: string): Promise<string | null> {
...
export function buildClaudeSessionFromText(input: { path: string; sessionId: string; text: string }): ClaudeCliSession {
...
export class ClaudeCliClient implements ClaudeCliGateway {
  ...
  sendMessage(
    ...
    const prompt = params.prompt.endsWith('\n') ? params.prompt : `${params.prompt}\n`
    const proc = this.runner.start({
      command: args[0],
      args: args.slice(1),
      cwd: params.cwd,
    ...
    const completion = (async () => {
      ...
      if (exit.code !== 0 && !interrupted) {
        const message = stderrBuffer.trim() || `claude exited with code ${exit.code}`
