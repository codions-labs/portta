...
type PipedCodexAppServerProcess = RunningProcess
...
const JsonValueSchema = z.union([
  ...
  z.record(z.string(), UnknownValueSchema),
])
const CodexAppServerContentItemSchema: z.ZodType<CodexAppServerContentItem> = z.object({
  type: z.string(),
  text: z.string().optional(),
})
const CodexAppServerUserMessageItemSchema: z.ZodType<CodexAppServerUserMessageItem> = z.object({
  type: z.literal('userMessage'),
  id: z.string(),
  content: z.array(CodexAppServerContentItemSchema),
})
const CodexAppServerAgentMessageItemSchema: z.ZodType<CodexAppServerAgentMessageItem> = z.object({
...
const CodexAppServerCommandActionSchema: z.ZodType<CodexAppServerCommandAction> = z.object({
  type: z.string(),
  command: z.string().optional(),
  path: z.string().nullable().optional(),
})
const CodexAppServerCommandExecutionItemSchema: z.ZodType<CodexAppServerCommandExecutionItem> = z.object({
...
const CodexAppServerPatchChangeKindSchema: z.ZodType<CodexAppServerPatchChangeKind> = z.union([
  z.object({ type: z.literal('add') }),
  z.object({ type: z.literal('delete') }),
  z.object({ type: z.literal('update'), move_path: z.string().nullable() }),
])
const CodexAppServerFileUpdateChangeSchema: z.ZodType<CodexAppServerFileUpdateChange> = z.object({
  path: z.string(),
  kind: CodexAppServerPatchChangeKindSchema,
  diff: z.string(),
})
const CodexAppServerFileChangeItemSchema: z.ZodType<CodexAppServerFileChangeItem> = z.object({
  type: z.literal('fileChange'),
  id: z.string(),
  changes: z.array(CodexAppServerFileUpdateChangeSchema),
  status: z.enum(['inProgress', 'completed', 'failed', 'declined']),
})
const CodexAppServerMcpToolCallResultSchema: z.ZodType<CodexAppServerMcpToolCallResult> = z.object({
  content: z.array(JsonValueSchema),
  structuredContent: JsonValueSchema,
  _meta: JsonValueSchema,
})
const CodexAppServerMcpToolCallErrorSchema: z.ZodType<CodexAppServerMcpToolCallError> = z.object({
  message: z.string(),
})
const CodexAppServerMcpToolCallItemSchema: z.ZodType<CodexAppServerMcpToolCallItem> = z.object({
...
const CodexAppServerDynamicToolCallContentItemSchema: z.ZodType<CodexAppServerDynamicToolCallContentItem> = z.union([
  z.object({ type: z.literal('inputText'), text: z.string() }),
  z.object({ type: z.literal('inputImage'), imageUrl: z.string() }),
])
const CodexAppServerDynamicToolCallItemSchema: z.ZodType<CodexAppServerDynamicToolCallItem> = z.object({
...
const CodexAppServerWebSearchActionSchema: z.ZodType<CodexAppServerWebSearchAction> = z.union([
...
const CodexAppServerWebSearchItemSchema: z.ZodType<CodexAppServerWebSearchItem> = z.object({
  type: z.literal('webSearch'),
  id: z.string(),
  query: z.string(),
  action: CodexAppServerWebSearchActionSchema.nullable(),
})
const CodexAppServerIgnoredItemSchema: z.ZodType<CodexAppServerIgnoredItem> = z.object({
  type: z.enum([
    'hookPrompt',
    'plan',
    'reasoning',
...
const CodexAppServerGenericItemSchema: z.ZodType<CodexAppServerGenericItem> = z.object({
  type: z.string(),
  id: z.string(),
})
const CodexAppServerThreadItemSchema: z.ZodType<CodexAppServerThreadItem> = z.union([
...
const CodexAppServerTurnSchema: z.ZodType<CodexAppServerTurn> = z
  .object({
    id: z.string(),
...
const CodexAppServerThreadStatusSchema: z.ZodType<CodexAppServerThreadStatus> = z.object({
  type: z.string(),
  activeFlags: z.array(z.string()).optional(),
})
const CodexAppServerThreadSchema: z.ZodType<CodexAppServerThread> = z
  .object({
    id: z.string(),
...
const CodexAppServerThreadListResponseSchema: z.ZodType<CodexAppServerThreadListResponse> = z.object({
  data: z.array(CodexAppServerThreadSchema),
  nextCursor: z.string().nullable(),
})
const CodexAppServerThreadReadResponseSchema: z.ZodType<CodexAppServerThreadReadResponse> = z.object({
  thread: CodexAppServerThreadSchema,
})
const CodexAppServerThreadContextSchema: z.ZodType<CodexAppServerThreadContext> = z.object({
  thread: CodexAppServerThreadSchema,
  model: z.string(),
  modelProvider: z.string(),
...
const CodexAppServerTurnStartResponseSchema: z.ZodType<CodexAppServerTurnStartResponse> = z.object({
...
export function readCodexAppServerStdoutLines(input: {
  ...
  let buffer =
    input.buffer + (input.chunk ? input.decoder.decode(input.chunk, { stream: true }) : input.decoder.decode())
  const lines: string[] = []
...
export class CodexAppServerClient implements CodexAppServerGateway {
  ...
  private startProcess(): void {
    ...
    const proc = this.runner.start({ command: 'codex', args: ['app-server'], keepStdinOpen: true })

    this.proc = proc
    this.startStdoutLoop(proc)
    this.startStderrLoop(proc)
    proc.exited.then((exit) => {
      const message = `codex app-server exited with code ${exit.code}`
  ...
  private handleResponse(id: number, raw: Record<string, unknown>): void {
    ...
    if (responseError) {
      pending.reject(new CodexAppServerRequestError(responseError.message, responseError.code, responseError.data))
      return
    }
  ...
  private async requestInternal<T>(method: string, schema: z.ZodType<T>, params?: unknown): Promise<T> {
    if (!this.proc) {
  ...
  private send(payload: unknown): void {
    ...
    void proc.writeStdin(this.encoder.encode(`${JSON.stringify(payload)}\n`)).catch((error: unknown): void => {
