...
import type { PaneSplit } from 'portta-core/taskflow'
import { CLI_NAME } from 'portta-core/taskflow/config'
import { log } from '../lib/log.ts'
import { isRecord } from '../lib/type-guards.ts'
import { stripProjectEnv } from './project-env.ts'
import { parsePaneTarget, type SessionGateway, type SessionWindowSummary } from './session-gateway.ts'
...
function parseHerdrResponse(value: unknown): HerdrResponse {
...
/** herdr addresses panes as `w<N>:p<M>` and tabs as `w<N>:t<M>`. taskflow pane
...
export function resolveSocketPath(env: Record<string, string | undefined>): string {
  ...
  return session ? `${home}/.config/herdr/sessions/${session}/herdr.sock` : `${home}/.config/herdr/herdr.sock`
}
 ...
 *  first (only `events.subscribe` is long-lived, which taskflow does not use here). */
async function request(socketPath: string, method: string, params: Record<string, unknown>): Promise<HerdrResponse> {
  return await new Promise<HerdrResponse>((resolve, reject) => {
    ...
    socket.on('data', (chunk: Uint8Array): void => {
      ...
      clearTimeout(timer)
      try {
        settle(() => resolve(parseHerdrResponse(JSON.parse(line))))
      } catch {
...
function parsePanes(result: Record<string, unknown> | undefined): PaneSummary[] {
  return asArray(result?.panes).flatMap((entry) => {
    ...
    const workspaceId = readString(record, 'workspace_id')
    return id && tabId && workspaceId ? [{ pane_id: id, tab_id: tabId, workspace_id: workspaceId }] : []
  })
}
 ...
 *
 *  taskflow's session/window/pane vocabulary maps onto herdr as
 *  session→workspace (matched by label), window→tab (matched by label),
 *  pane→pane. Labels are the join key because taskflow persists names, not ids,
...
export class HerdrGateway implements SessionGateway {
  ...
  async ensureServer(): Promise<void> {
    ...
    // `server_not_running`. taskflow has to start and detach the server itself.
    ...
    throw new Error(`herdr server did not come up at ${this.socketPath} within ${SERVER_START_TIMEOUT_MS}ms`)
  }
    ...
    if (typeof protocol === 'number' && protocol !== EXPECTED_PROTOCOL) {
      log.warn(
        `[herdr] protocol ${protocol} differs from the expected ${EXPECTED_PROTOCOL}; ` +
          `pane orchestration may misbehave — upgrade ${CLI_NAME} or pin herdr`,
      )
    }
  }
  ...
  /** Resolve any target taskflow hands the gateway to a concrete herdr pane id:
    ...
    const summaries = await Promise.all(
      workspaces.map(async (workspace) => {
        ...
        return tabs.flatMap((tab) =>
          tab.label ? [{ sessionName: workspaceLabel, windowName: tab.label, paneCount: tab.pane_count ?? 0 }] : [],
        )
      }),
    )
