...
/** Multiplexer-agnostic control surface taskflow drives to build, inspect and tear
...
export interface SessionGateway {
  ...
  /** Create a window and apply whatever backend-specific options taskflow relies on
   *  (pane indexing, rename suppression). Replaces any existing window of the
   *  same name only if the caller killed it first. */
  createWindow(opts: { sessionName: string; windowName: string; cwd: string; command?: string }): Promise<void>
  splitWindow(opts: {
    target: string
    split: PaneSplit
    sizePct?: number
  ...
  runCommand(target: string, command: string, options?: RunCommandOptions): Promise<void>
...
export function buildProjectSessionName(projectRoot: string): string {
  ...
  return `${RUNTIME_IDENTITY.tmuxPrefix}-${base}-${hash}`
}

export function buildWorktreeWindowName(branch: string): string {
  return `${RUNTIME_IDENTITY.tmuxPrefix}-${branch}`
}
...
export function buildWorktreeParkingWindowName(branch: string): string {
  return `${RUNTIME_IDENTITY.tmuxPrefix}-${branch}-tabs`
}
