...
import { hostPortFromEnv, RUNTIME_IDENTITY } from 'portta-core/taskflow/config'
import { log } from '../lib/log.ts'
import { NodeProcessRunner } from './process-runner.ts'
import { stripProjectEnv } from './project-env.ts'
...
const defaultSpawnPtyProcess: SpawnPtyProcess = (args, env) => {
  ...
  if (!command) throw new Error('PTY command is required')
  const process = processRunner.start({ command, args: commandArgs, env, keepStdinOpen: true })
  if (process.pid === undefined) throw new Error('PTY process failed to start')
  return {
    pid: process.pid,
    stdin: { write: (data: Uint8Array): Promise<void> => process.writeStdin(data) },
    stdout: process.stdout,
    stderr: process.stderr,
    exited: process.exited.then((exit): number => exit.code ?? -1),
    kill: (): void => {
      process.kill()
    },
  }
...
const defaultSpawnTmuxProcess: SpawnTmuxProcess = (args, opts = {}) => {
  const [command, ...commandArgs] = args
  if (!command) throw new Error('tmux command is required')
  const runningProcess = processRunner.start({
    command,
    args: commandArgs,
  ...
  void new Response(runningProcess.stdout).arrayBuffer()
  return {
    stderr: runningProcess.stderr,
    exited: runningProcess.exited.then((exit): number => exit.code ?? -1),
    kill: (): void => {
      runningProcess.kill()
    },
  }
}
const defaultSleep: Sleep = (ms) => sleepFor(ms)

const defaultSpawnSyncCommand: SpawnSyncCommand = (args, opts = {}) => {
  const [command, ...commandArgs] = args
  if (!command) throw new Error('Command is required')
  const result = spawnSync(command, commandArgs, {
    env: opts.env ?? stripProjectEnv(process.env),
...
export function setTerminalAdapterDependenciesForTests(
  deps: {
...
const DASH_PORT = String(hostPortFromEnv(process.env))
const SESSION_PREFIX = `${RUNTIME_IDENTITY.terminalSessionPrefix}-${DASH_PORT}-`
const MAX_SCROLLBACK_BYTES = 1 * 1024 * 1024 // 1 MB
const TMUX_TIMEOUT_MS = 5_000
const sessions = new Map<string, TerminalSession>()
const attachmentsByTarget = new Map<string, string>()
...
async function tmuxExec(
  ...
  const timeout = sleepFor(TMUX_TIMEOUT_MS).then(() => {
    proc.kill()
...
/** Kill orphaned dashboard tmux sessions left from previous server runs. */
export function cleanupStaleSessions(): void {
  try {
    const result = spawnSyncCommand(['tmux', 'list-sessions', '-F', '#{session_name}'], {
      stdout: 'pipe',
      stderr: 'pipe',
    })
...
export async function attach(
  ...
  const targetKey = `${target.ownerSessionName}:${target.windowName}`
  log.debug(
    `[term] attach(${attachId}) target=${targetKey} cols=${cols} rows=${rows} existing=${sessions.has(attachId)} active=${attachmentsByTarget.get(targetKey) ?? 'none'}`,
  )
  if (sessions.has(attachId)) {
    await detach(attachId)
  }
  const previousAttachId = attachmentsByTarget.get(targetKey)
  if (previousAttachId && previousAttachId !== attachId) {
    log.debug(`[term] attach(${attachId}) replacing previous attachment=${previousAttachId} target=${targetKey}`)
    await detach(previousAttachId)
  }
  ...
  const proc = spawnPtyProcess(buildPtyArgs(cmd), { ...stripProjectEnv(process.env), TERM: 'xterm-256color' })
  ...
  attachmentsByTarget.set(targetKey, attachId)
  ...
  proc.exited.then((exitCode) => {
    ...
    if (sessions.get(attachId) === session) {
      ...
      if (attachmentsByTarget.get(session.targetKey) === attachId) attachmentsByTarget.delete(session.targetKey)
    } else {
...
export function write(attachId: string, data: string): void {
  ...
  try {
    void session.proc.stdin.write(textEncoder.encode(data)).catch((err: unknown): void => {
...
export async function resize(attachId: string, cols: number, rows: number): Promise<void> {
  const previous = resizeChains.get(attachId) ?? Promise.resolve()
  const next = previous.then(async (): Promise<void> => {
    const session = sessions.get(attachId)
    if (!session) return
    const windowTarget = `${session.groupedSessionName}:${session.windowName}`
    log.debug(`[term] resize(${attachId}) target=${windowTarget} cols=${cols} rows=${rows}`)
  ...
  resizeChains.set(attachId, next)
  try {
    await next
  } finally {
    if (resizeChains.get(attachId) === next) resizeChains.delete(attachId)
  }
}
...
export async function sendPrompt(
  ...
  if (preamble) {
    ...
    if (preambleResult.exitCode !== 0) {
      return {
        ok: false,
        error: `send-keys preamble failed${preambleResult.stderr ? `: ${preambleResult.stderr}` : ''}`,
      }
    }
  }

  const cleaned = text.replace(/\0/g, '')
  const bufferName = `${RUNTIME_IDENTITY.tmuxBufferPrefix}-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`
