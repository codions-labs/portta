...
import { leakedProjectEnvKeys, stripProjectEnv } from './project-env.ts'
import {
  buildWindowTarget,
...
/** Window options every taskflow-created window needs: zero-based pane indexing
...
function tmuxSpawnEnv(): Record<string, string> {
  return stripProjectEnv(process.env)
}

export function synchronizeTmuxPath(path: string): void {
  const base = tmuxSpawnEnv()
  spawnSync('tmux', ['set-environment', '-g', 'PATH', path], {
    stdio: 'ignore',
...
function detectUtf8Locale(): string {
  ...
  try {
    const result = spawnSync('locale', ['-a'], { encoding: 'utf8', stdio: ['ignore', 'pipe', 'pipe'] })
    if (result.status === 0) {
 ...
 *  output as `_`, which silently breaks taskflow's tab-delimited parsing of
...
function runTmux(args: string[]): { stdout: string; stderr: string; exitCode: number } {
  // Pass an explicit env — spawnSync treats it as the complete child
...
export class TmuxGateway implements SessionGateway {
   ...
   *  taskflow that predates the stripped-env spawn). Removing them from the global
  ...
  async createWindow(opts: { sessionName: string; windowName: string; cwd: string; command?: string }): Promise<void> {
    ...
    // Pane targets taskflow builds are zero-based and window names are meaningful,
  ...
  async runCommand(target: string, command: string, options: RunCommandOptions = {}): Promise<void> {
    if (options.replaceShell) {
      const shell = process.env.SHELL || '/bin/bash'
      const launch = `${command}; exec ${quoteShell(shell)} -i`
      assertTmuxOk(['respawn-pane', '-k', '-t', target, launch], `launch tmux command in ${target}`)
      return
    }
  ...
  async createParkedPane(opts: {
    ...
    return assertTmuxOk(
      [
        ...
        '#{pane_id}',
        '-t',
        buildWindowTarget(opts.sessionName, opts.parkingWindow),
        '-c',
        opts.cwd,
        opts.command,
      ],
  ...
  async killPane(target: string): Promise<void> {
    ...
    if (
      ...
      !isIgnorableKillWindowError(result.stderr)
    ) {
