import type { AgentDetails, AgentSummary } from 'portta-contracts/taskflow'
import type { AgentId, AgentKind, CustomAgentConfig, ProjectConfig } from 'portta-core/taskflow'
...
export function listAgentDefinitions(config: Pick<ProjectConfig, 'agents'>): AgentDefinition[] {
  ...
  return [...BUILTIN_AGENT_DEFINITIONS.map((definition) => cloneDefinition(definition)), ...customDefinitions]
}

export function getAgentDefinition(config: Pick<ProjectConfig, 'agents'>, agentId: AgentId): AgentDefinition | null {
