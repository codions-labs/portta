...
import { buildCodexItemConversationMessages } from './worktree-conversation-service.ts'
...
export function buildAgentsUiMessageUpsertEvents(
  ...
  const createdAt = toIsoTimestampMs(
    notification.method === 'item/started' ? readNumber(params.startedAtMs) : readNumber(params.completedAtMs),
  )
...
export class AgentsConversationStreamSession {
  ...
  handleLiveMessageUpsert(event: AgentsUiConversationMessageUpsertLivePayload, orderKey = event.message.id): void {
    if (this.closed || event.conversationId !== this.conversationId) return
