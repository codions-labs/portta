import type { WorktreeArchiveState } from 'portta-core/taskflow'
import { readWorktreeArchiveState, writeWorktreeArchiveState } from '../adapters/fs.ts'
import { pruneArchivedWorktreeState, setArchivedWorktreeState } from './archive-service.ts'

function archiveStatesEqual(left: WorktreeArchiveState, right: WorktreeArchiveState): boolean {
  if (left.schemaVersion !== right.schemaVersion) return false
  if (left.entries.length !== right.entries.length) return false

  return left.entries.every(
    (entry, index) =>
      entry.path === right.entries[index]?.path && entry.archivedAt === right.entries[index]?.archivedAt,
  )
}
...
export class ArchiveStateService {
  ...
  constructor(gitDir: string, deps: ArchiveStateServiceDependencies = {}) {
    this.gitDir = gitDir
