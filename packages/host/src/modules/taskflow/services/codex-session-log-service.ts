...
import type { AgentsUiConversationMessage } from 'portta-contracts/taskflow'
import type { CodexAppServerThread } from '../adapters/codex-app-server.ts'
...
function readMessageText(raw: unknown): string {
  if (!Array.isArray(raw)) return ''
  return raw
...
export function parseCodexSessionMessages(text: string): AgentsUiConversationMessage[] {
  ...
  for (const line of text.split('\n')) {
    ...
    if (payloadType === 'message') {
      const role = record.payload.role === 'user' || record.payload.role === 'assistant' ? record.payload.role : null
      const text = readMessageText(record.payload.content)
      if (!role || !text) continue
      const phase = role === 'assistant' ? (readString(record.payload.phase) ?? undefined) : undefined
      if (hasDuplicateTextMessage({ messages, turnId: currentTurnId, role, text, phase })) continue
      pushMessage(messages, {
        id: readString(record.payload.id) ?? `${role}:${currentTurnId}:${blockIndex}`,
        turnId: currentTurnId,
        role,
        kind: phase === 'analysis' ? 'thinking' : 'text',
        ...(phase ? { phase } : {}),
      ...
      const argumentsText =
        payloadType === 'custom_tool_call'
          ? typeof record.payload.input === 'string'
            ...
            : compactJson(record.payload.input ?? {})
          : typeof record.payload.arguments === 'string'
            ? record.payload.arguments
            : compactJson(record.payload.arguments ?? {})
      const command = readToolCommand(toolName, argumentsText)
...
export async function readCodexSessionMessages(thread: CodexAppServerThread): Promise<AgentsUiConversationMessage[]> {
  ...
  try {
    return parseCodexSessionMessages(await readFile(thread.path, 'utf8'))
  } catch {
    return []
  }
}
