import { LINEAR_IDENTITY } from 'portta-core/taskflow/config'
import type { GitGateway } from '../adapters/git.ts'
import { type SerializedIntervalDependencies, startSerializedInterval } from '../lib/async.ts'
import { log } from '../lib/log.ts'
import type { CreateLifecycleWorktreeInput } from './lifecycle-service.ts'
import { branchMatchesIssue, fetchAssignedIssues, type LinearIssue } from './linear-service.ts'
...
export interface LinearAutoCreateDependencies {
  ...
  /** Optional handler for the `taskflow_oneshot` label variant. Must return the actual
   ...
   *  Only fires for the `taskflow_oneshot` path: regular `taskflow` pickups are
...
const AUTO_CREATE_LABEL = LINEAR_IDENTITY.label
const AUTO_ONESHOT_LABEL = LINEAR_IDENTITY.oneshotLabel
...
/** Filter issues to only those in Todo state with the "taskflow" label that don't already
...
/** Filter issues to only those in Todo state with the "taskflow_oneshot" label that don't already
 *  have a worktree. The "taskflow_oneshot" label wins over "taskflow" — issues tagged with both
...
export async function runLinearAutoCreateOnce(deps: LinearAutoCreateDependencies): Promise<void> {
  ...
  const eligibleIssueIds = new Set(
    result.data
      .filter(
        (issue) =>
          issue.state.name === 'Todo' && (hasLabel(issue, AUTO_CREATE_LABEL) || hasLabel(issue, AUTO_ONESHOT_LABEL)),
      )
      .map((issue) => issue.id),
  )
  ...
  if (oneshotIssues.length === 0 && createIssues.length === 0) {
    log.debug(
      `[linear-auto-create] no new labeled issues (${result.data.length} assigned, ${existingBranches.length} worktrees)`,
    )
    return
  }
  ...
  if (oneshotIssues.length > 0) {
    ...
    for (const issue of oneshotIssues) {
      try {
        ...
        const runOneshotForIssue = deps.runOneshotForIssue
        if (!runOneshotForIssue) continue
        const { branch } = await runOneshotForIssue(issue.identifier)
  ...
  if (createIssues.length > 0) {
    ...
    for (const issue of createIssues) {
      try {
        ...
        // No Linear pickup comment for the regular `taskflow` path — the user
