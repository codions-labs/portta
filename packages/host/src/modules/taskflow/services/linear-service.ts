...
export type { LinkedLinearIssue } from 'portta-core/taskflow'
...
export type FetchIssuesResult = { ok: true; data: LinearIssue[] } | { ok: false; error: string }

export type BuildLinearIssuesResponseResult = { ok: true; data: LinearIssuesResponse } | { ok: false; error: string }
...
export type CreateLinearIssueResult = { ok: true; data: CreatedLinearIssue } | { ok: false; error: string }

type FetchViewerIdResult = { ok: true; data: string } | { ok: false; error: string }

type FetchStateIdResult = { ok: true; data: string } | { ok: false; error: string }
...
function parseInProgressStateIdResponse(raw: GqlResponse<TeamStatesQueryData>): FetchStateIdResult {
  ...
  const preferredState = states.find(
    (state) => state.type === 'started' && state.name.trim().toLowerCase() === 'in progress',
  )
  if (preferredState) {
...
export function branchMatchesIssue(worktreeBranch: string, issueBranchName: string): boolean {
...
/**
 * Exported so the daemon's forge surface can reach Linear through this client
 * rather than opening a second one. Taskflow's own queries stay in this file;
 * the forge's queries live beside the routes that need them, and both go out
 * over the same key, the same endpoint and the same error shape.
 */
...
// ── Issue + attachments query (for --linear / `taskflow linear post`) ─────────
...
export type SearchTeamIssuesResult = { ok: true; data: LinearIssue[] } | { ok: false; error: string }
...
export async function searchTeamIssuesByKeywords(input: {
  ...
  const response = await postLinearGraphql<TeamIssuesByKeywordsQueryData>(TEAM_ISSUES_BY_KEYWORDS_QUERY, {
    teamId: input.teamId,
    titleFilters,
...
export type UploadFileResult = { ok: true; data: { assetUrl: string } } | { ok: false; error: string }

export type AttachToIssueResult = { ok: true; data: { id: string; url: string } } | { ok: false; error: string }

export type CreateCommentResult = { ok: true; data: { id: string; url: string } } | { ok: false; error: string }

export { type LinearTarget, parseLinearTarget } from 'portta-contracts/taskflow'
export function buildTaskflowAttachmentTitle(branch: string): string {
  return `${LINEAR_IDENTITY.attachmentTitlePrefix}${branch}`
}

export function findTaskflowAttachment(
  issue: { attachments: LinearAttachment[] },
  branch?: string,
): LinearAttachment | null {
  const candidates = issue.attachments.filter((a) => a.title.startsWith(LINEAR_IDENTITY.attachmentTitlePrefix))
  if (candidates.length === 0) return null
  if (branch) {
    const exact = candidates.find((a) => a.title === buildTaskflowAttachmentTitle(branch))
...
export function findLinkedGitHubPr(issue: { attachments: LinearAttachment[] }): LinkedGitHubPr | null {
  ...
  const indexed = githubAttachments.map((attachment, idx) => {
    const pr: LinkedGitHubPr = {
      url: attachment.url,
      branch: inferPrBranchFromAttachment(attachment),
      state: inferPrStateFromAttachment(attachment),
    }
    return { pr, idx, attachment }
  })
  indexed.sort((a, b) => {
 ...
 *  `taskflow_oneshot`-labeled issue. The prefix (`**Taskflow pickup — branch ...**`) is the
...
export function buildLinearPickupMarkdown(input: LinearPickupMarkdownInput): string {
  return [
    `**${APP_NAME} pickup — branch \`${input.branch}\`**`,
    '',
...
export function buildLinearSummaryMarkdown(input: LinearSummaryInput): string {
  const lines: string[] = [`**${APP_NAME} session — branch \`${input.branch}\`**`, '']
  ...
  if (input.taskflowVersion) lines.push(`- ${CLI_NAME}: ${input.taskflowVersion}`)
  lines.push('')
  lines.push(`_Resume on another machine with_ \`${CLI_NAME} oneshot --linear <issue-id>\`.`)
  return lines.join('\n')
}
...
export async function uploadAttachmentFile(input: {
  ...
  // The GCS pre-signed URL signs the exact header values, and a `fetch` that
  // normalizes them (e.g. appends `;charset=utf-8` to an application/json
  // Content-Type) causes SignatureDoesNotMatch. node:https sends them byte-exact.
  try {
...
export async function createIssueComment(input: { issueId: string; body: string }): Promise<CreateCommentResult> {
