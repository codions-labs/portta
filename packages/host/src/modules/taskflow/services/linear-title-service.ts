...
import { type LinearIssue, searchTeamIssuesByKeywords } from './linear-service.ts'
import { llmProviderLabel, type RunLlmResult, runShortLlmTask } from './llm-spawn.ts'
...
export async function polishLinearIssueTitle(input: PolishLinearIssueTitleInput): Promise<PolishedTitleResult | null> {
  ...
  try {
    result = await runLlm(input.autoName, POLISH_SYSTEM_PROMPT, buildPolishUserPrompt(input.prompt.trim()), {
...
// same way `taskflow oneshot --linear` does, so the web and CLI surfaces produce
...
export async function findDuplicateLinearIssue(input: FindDuplicateLinearIssueInput): Promise<LinearIssue | null> {
...
function buildDedupUserPrompt(input: { polishedTitle: string; prompt: string; candidates: LinearIssue[] }): string {
  const list = input.candidates.map((c) => `${c.identifier}: ${c.title}`).join('\n')
  const fullPrompt = input.prompt.trim()
  const codePoints = [...fullPrompt]
  const excerpt =
    codePoints.length > MAX_DEDUP_PROMPT_EXCERPT
