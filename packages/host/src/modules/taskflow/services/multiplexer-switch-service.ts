...
export interface MultiplexerSwitchDependencies {
  ...
  /** Refuse the switch while a Taskflow Run is still mutating a workspace. */
  hasActiveRuns?: () => boolean | Promise<boolean>
 ...
 *  has flipped, taskflow only speaks to the new multiplexer and can no longer reach
...
export async function switchMultiplexer(
  ...
  if (await deps.hasActiveRuns?.()) {
    return { ok: false, error: 'cannot switch multiplexer while a Run is active', failures: [] }
  }
