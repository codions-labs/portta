import type { RuntimeEvent } from 'portta-core/taskflow'
...
function eventToNotificationInput(
  event: RuntimeEvent,
): { branch: string; type: RuntimeNotification['type']; message: string; url?: string } | null {
  switch (event.type) {
    case 'agent_stopped':
