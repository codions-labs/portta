import type { CiCheck, LinkedRepoConfig, PrComment, PrEntry } from 'portta-core/taskflow'
import { readWorktreePrs, writeWorktreePrs } from '../adapters/fs.ts'
import { NodeProcessRunner, runCaptured } from '../adapters/process-runner.ts'
import { mapWithConcurrency, startSerializedInterval } from '../lib/async.ts'
import { log } from '../lib/log.ts'

const PR_FETCH_LIMIT = 50
const GH_TIMEOUT_MS = 15_000
const ghRunner = new NodeProcessRunner()
...
async function runGh(
  ...
  if (command === undefined) throw new Error('gh command is required')
  const result = await runCaptured(ghRunner, {
    command,
    args: args.slice(1),
    cwd,
    timeoutMs: GH_TIMEOUT_MS,
  })
  return { stdout: result.stdout, stderr: result.stderr, code: result.exit.code, timedOut: result.exit.timedOut }
}

// ── Internal GH API shapes ────────────────────────────────────────────────────

type GhCheckStatus = 'QUEUED' | 'IN_PROGRESS' | 'COMPLETED' | 'WAITING' | 'REQUESTED' | 'PENDING'
...
type FetchPrsResult = { ok: true; data: Map<string, PrEntry> } | { ok: false; error: string }
...
export function summarizeChecks(checks: GhCheckEntry[] | null): PrEntry['ciStatus'] {
  ...
  const allDone = relevant.every((c) =>
    c.__typename === 'StatusContext' ? c.state !== 'PENDING' && c.state !== 'EXPECTED' : c.status === 'COMPLETED',
  )
...
export function parsePrResponse(json: string, repoLabel?: string): Map<string, PrEntry> {
 ...
 */
export async function fetchAllPrs(repoSlug?: string, repoLabel?: string, cwd?: string): Promise<FetchPrsResult> {
  ...
  if (result.code !== 0) {
    return {
      ok: false,
      error: `gh pr list failed for ${label} (exit ${result.code}): ${result.stderr.trim()}`,
    }
  }

  try {
    return { ok: true, data: parsePrResponse(result.stdout, repoLabel) }
  } catch (err) {
...
async function fetchReviewComments(prNumber: number, repoSlug?: string, cwd?: string): Promise<PrComment[]> {
  const repoFlag = repoSlug ? repoSlug : '{owner}/{repo}'
  const apiPath = `repos/${repoFlag}/pulls/${prNumber}/comments?per_page=100`
  const args = ['gh', 'api', apiPath, '--include']
  ...
  const result = await runGh(args, cwd)
  if (result.timedOut) return cached?.comments ?? []
  ...
  if (blankLineIdx === -1) {
    ...
    if (result.code !== 0) return cached?.comments ?? []
    try {
  ...
  const etag = headerBlock.match(/^etag:\s*(.+)$/im)?.[1]

  try {
    const comments = parseReviewComments(body)
    if (etag) {
      etagCache.set(apiPath, { etag: etag.trim(), comments })
    }
    return comments
  } catch {
    return cached?.comments ?? []
...
async function fetchPrStatus(url: string): Promise<PrStatus | null> {
  const result = await runGh(['gh', 'pr', 'view', url, '--json', 'state,isDraft'])
  if (result.timedOut || result.code !== 0) return null

  return parsePrViewStatus(result.stdout)
}
...
/** Sync PR status into per-worktree taskflow storage for all worktrees that have open PRs. */
export async function syncPrStatus(
  ...
  if (reviewTuples.length > 0) {
    ...
    for (const [i, { entry }] of reviewTuples.entries()) {
...
export function startPrMonitor(
  ...
  const run = async (): Promise<void> => {
    ...
    await syncPrStatus(getWorktreeGitDirs, linkedRepos, projectDir).catch((err: unknown) => {
...
export function startAutoRemoveMonitor(run: () => Promise<void>, intervalMs: number = 60_000): () => void {
