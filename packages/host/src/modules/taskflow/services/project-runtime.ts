...
import { buildWorktreeWindowName } from '../adapters/session-gateway.ts'
...
function makeDefaultState(input: {
  ...
  return {
    ...
    interfaceMode: input.interfaceMode ?? 'terminal',
...
export class ProjectRuntime {
  ...
  upsertWorktree(input: {
    ...
    if (existing) {
      ...
      if (input.interfaceMode) existing.interfaceMode = input.interfaceMode
      ...
      if (input.issueRef !== undefined) existing.issueRef = input.issueRef
  ...
  setGitState(worktreeId: string, patch: Partial<ManagedWorktreeRuntimeState['git']>): ManagedWorktreeRuntimeState {
    const state = this.requireWorktree(worktreeId)
  ...
  private applyRuntimeError(state: ManagedWorktreeRuntimeState, event: RuntimeErrorEvent, timestamp: string): void {
