...
import { writeOpenSessionsState } from '../adapters/fs.ts'
...
export function computeOpenBranches(input: {
  ...
  const openWindowNames = new Set(
    input.windows.filter((window) => window.sessionName === input.sessionName).map((window) => window.windowName),
  )

  return input.worktrees
    .filter((entry) => !entry.bare && !sameFsPath(entry.path, resolvedProjectDir))
...
/** Persist the currently-open worktree sessions for `taskflow restore`.
 *
...
export async function saveOpenSessionsSnapshot(deps: SessionSnapshotDependencies): Promise<string[] | null> {
