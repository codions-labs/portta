...
import type { PaneKind, PaneTemplate } from 'portta-core/taskflow'
import type { SessionGateway } from '../adapters/session-gateway.ts'
import { buildPaneTarget, buildProjectSessionName, buildWorktreeWindowName } from '../adapters/session-gateway.ts'
...
function resolvePaneLaunchCommand(template: PaneTemplate, ctx: SessionLayoutContext): string {
  return template.kind === 'runtime' ? ctx.paneCommands.runtime : ctx.paneCommands.shell
}
...
export function planSessionLayout(
  ...
  const panes = templates.map((template, index) => {
    ...
    return {
      ...
      launchCommand: resolvePaneLaunchCommand(template, ctx),
...
export async function isWorktreeOpen(sessions: SessionGateway, projectRoot: string, branch: string): Promise<boolean> {
...
export async function ensureSessionLayout(sessions: SessionGateway, plan: SessionLayoutPlan): Promise<void> {
  const rootPane = plan.panes[0]
  if (!rootPane) throw new Error('Session layout plan has no panes')
  ...
  await sessions.createWindow({
    ...
    cwd: rootPane.cwd,
    command: rootPane.launchCommand,
  })
