...
} from '../adapters/codex-app-server.ts'
import { readWorktreeMeta, writeWorktreeMeta } from '../adapters/fs.ts'
import type { GitGateway } from '../adapters/git.ts'
import { log } from '../lib/log.ts'
import { isRecord } from '../lib/type-guards.ts'
import { buildAgentsUiWorktreeSummary } from './agents-ui-service.ts'
import { readCodexSessionMessages } from './codex-session-log-service.ts'
import { err, ok, type WorktreeConversationResult } from './worktree-conversation-result.ts'
...
export interface WorktreeConversationServiceDependencies {
  appServer: Pick<
    import('../adapters/codex-app-server.ts').CodexAppServerGateway,
    'threadList' | 'threadRead' | 'threadResume' | 'threadStart' | 'turnStart' | 'turnInterrupt'
  >
  git: Pick<GitGateway, 'resolveWorktreeGitDir'>
  resolveLaunchContext: (
    input: ResolveCodexAppServerLaunchContextInput,
  ) =>
    | Promise<WorktreeConversationResult<CodexAppServerLaunchContext>>
...
export function resolveCodexAppServerLaunchContext(input: {
  ...
  if (input.meta.runtime !== 'host' || input.profile.runtime !== 'host') {
    return err(
      409,
      'Codex web chat is only available for host-runtime worktrees. Use the terminal for Docker worktrees.',
    )
  }

  return input.profile.yolo === true
    ? ok({
...
function isCodexConversationMeta(
  meta: WorktreeConversationMeta | null | undefined,
): meta is CodexWorktreeConversationMeta {
...
function sameConversationMeta(
  left: WorktreeConversationMeta | null | undefined,
  right: WorktreeConversationMeta,
): boolean {
...
export class WorktreeConversationService {
  ...
  private readonly deps: WorktreeConversationServiceDependencies
  constructor(deps: WorktreeConversationServiceDependencies) {
    this.deps = deps
  ...
  async readWorktreeConversation(
    ...
    try {
      ...
      if (!meta) return err(409, 'Worktree metadata is missing')
      const threads = (
        await this.deps.appServer.threadList({
          cwd: worktree.path,
          limit: 20,
          sortKey: 'updated_at',
        })
      ...
      const thread = threads.find((candidate) => candidate.id === savedThreadId) ?? selectDiscoveredThread(threads)
      if (!thread) return err(404, 'No Codex thread could be resolved for this worktree')
      const { conversationMeta } = await this.persistConversationMeta(gitDir, meta, thread)
      const sessionMessages = await this.readSessionMessages(thread)
      return ok(toWorktreeConversationResponse(worktree, conversationMeta, thread, sessionMessages))
    } catch (error) {
  ...
  async sendWorktreeConversationMessage(
    ...
    return await this.withResolvedConversation(worktree, true, async ({ thread, launchContext }) => {
      const started = await this.deps.appServer.turnStart({
        ...
        approvalsReviewer: launchContext.approvalsReviewer,
  ...
  private async resolveConversation(
    ...
    const { conversationMeta, meta: nextMeta } = await this.persistConversationMeta(gitDir, meta, thread, now)
  ...
  private async persistConversationMeta(
    ...
    thread: CodexAppServerThread,
    now = this.now(),
  ): Promise<{ conversationMeta: CodexWorktreeConversationMeta; meta: WorktreeMeta }> {
    ...
    const nextMeta = sameConversationMeta(meta.conversation, conversationMeta)
      ? {
          ...
          conversation: {
            ...conversationMeta,
            lastSeenAt: meta.conversation?.lastSeenAt ?? conversationMeta.lastSeenAt,
          },
        }
      : { ...meta, conversation: conversationMeta }
    if (!sameConversationMeta(meta.conversation, conversationMeta)) await this.writeMeta(gitDir, nextMeta)
    return {
      conversationMeta: isCodexConversationMeta(nextMeta.conversation) ? nextMeta.conversation : conversationMeta,
      meta: nextMeta,
    }
  }
  ...
  private async resolveThread(
    ...
    if (savedThreadId) {
      ...
      const discoveredThread = selectDiscoveredThread(
        (
          await this.deps.appServer.threadList({
            cwd,
            limit: 20,
