...
import type { AgentId, RuntimeKind, SessionInterfaceMode } from 'portta-core/taskflow'
import {
  type ControlEnvMap,
  type OneshotMeta,
...
import { type CreateWorktreeMode, type GitGateway, NodeGitGateway } from '../adapters/git.ts'
import type { SessionGateway } from '../adapters/session-gateway.ts'
import { ensureSessionLayout, type SessionLayoutPlan } from './session-service.ts'
...
export async function createManagedWorktree(
  ...
  const git = deps.git ?? new NodeGitGateway()
...
export function mergeManagedWorktree(opts: MergeManagedWorktreeOptions, git: GitGateway = new NodeGitGateway()): void {
  git.mergeBranch({
    repoRoot: opts.repoRoot,
