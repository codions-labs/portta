...
import type { AgentStatus, AgentUsage, ProviderId } from '../dsl/types.ts'
...
export interface JournalMeta {
   ...
   * An absent field is read as 'v1' on resume, never as "nothing to compare".
   */
  keyVersion?: string
...
/** Root data dir: ~/.portta/state/host (PORTTA_HOME/state/host). */
export function dataRoot(roots: RunRoots = {}): string {
  return roots.dataRoot ?? globalPaths().root
}

export function runDir(runId: string, roots: RunRoots = {}): string {
  return join(dataRoot(roots), 'runs', runId)
}

export function journalPath(runId: string, roots: RunRoots = {}): string {
  return join(runDir(runId, roots), 'journal.jsonl')
}

export function ensureRunDir(runId: string, roots: RunRoots = {}): string {
  const dir = runDir(runId, roots)
  mkdirSync(dir, { recursive: true })
  return dir
}

/** Run ids that have a journal on disk (newest first), for "did you mean" resume errors. */
export function listRunIds(roots: RunRoots = {}): string[] {
  const dir = join(dataRoot(roots), 'runs')
...
export class JournalNotFoundError extends Error {
  ...
  readonly knownRunIds: string[]
  constructor(runId: string, knownRunIds: string[] = []) {
    const nearby = knownRunIds.slice(0, 5)
    super(`no journal found for run "${runId}"${nearby.length > 0 ? ` — known runs: ${nearby.join(', ')}` : ''}`)
    this.runId = runId
...
export class Journal {
  ...
  constructor(runId: string, roots: RunRoots = {}) {
    this.path = journalPath(runId, roots)
  ...
  static exists(runId: string, roots: RunRoots = {}): boolean {
    return existsSync(journalPath(runId, roots))
  }

  static load(runId: string, roots: RunRoots = {}): LoadedJournal {
    const path = journalPath(runId, roots)
...
export function checkResumePreconditions(
  ...
  if (!meta) return // a journal without a meta line (partial write) has nothing to check
  // An ABSENT keyVersion means v1 — not "skip the check". Skipping would resume with 100% key
  // misses: a full silent re-bill (the C1 failure mode).
...
export function writeResult(runId: string, value: unknown, roots: RunRoots = {}): void {
  const dir = ensureRunDir(runId, roots)
