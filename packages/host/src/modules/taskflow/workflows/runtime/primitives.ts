...
import type { AgentOpts, AgentResult, AgentSpec, PipelineStage, RunDefaults, WorkflowGlobals } from '../dsl/types.ts'
import { addUsage, BUILTIN_PROVIDER_IDS, emptyUsage } from '../dsl/types.ts'
import { type RetryOptions, withRetry } from '../worker/errors.ts'
import type { WorkerFactory, WorkerProgress } from '../worker/index.ts'
import { AgentError, AgentInterrupted } from '../worker/index.ts'
import { stripNullOptionals, validate } from '../worker/schema.ts'
import type { EventSink } from './events.ts'
import type { ExecutionPort } from './execution-port.ts'
import type { Journal, LoadedJournal, RunRoots } from './journal.ts'
import { branchKey, chainKey, explicitKey, keyedSpec, ROOT_KEY } from './keys.ts'
import { Semaphore } from './semaphore.ts'
import { AgentTranscript } from './transcript.ts'
import { createWorktree, findGitRoot, teardownWorktree, type Worktree } from './worktree.ts'
...
/**
 * Reject a provider id the Run cannot serve. Unlike the enums above the list is not closed: a Run on
 * the ACP transport knows every id in the project's provider registry, a native Run only the
 * builtins. Checked at spec resolution for the same reason as checkSpecEnum — an unknown provider
 * would otherwise only fail at the factory, and not at all under --fake.
 */
export function checkProvider(value: string | undefined, known: readonly string[]): void {
  if (value !== undefined && !known.includes(value)) {
    throw new WorkflowError(`invalid provider "${value}" — must be one of ${known.join(', ')}`)
  }
}
...
export interface RuntimeOpts {
  ...
  /** Provider ids this Run may name; defaults to the builtins. */
  ...
  /** Host-owned workspace leases. Required in Taskflow mode when a workflow requests worktree isolation. */
...
export class Runtime {
  ...
  private readonly o: RuntimeOpts
  constructor(o: RuntimeOpts) {
    this.o = o
  ...
  private resolveSpec(prompt: string, opts: AgentOpts | undefined): AgentSpec {
    ...
    checkProvider(spec.provider, this.o.knownProviders ?? BUILTIN_PROVIDER_IDS)
  ...
  private async agentImpl<T = string>(prompt: string, opts?: AgentOpts): Promise<T> {
    ...
    if (opts?.worktree && this.o.taskflowMode && !this.o.executionPort) {
      throw new WorkflowError(
        `worktree is unavailable in ${APP_NAME} mode without an ExecutionPort fork lease because ${APP_NAME} owns workspace lifecycle`,
      )
    }
    if (opts?.key) {
    ...
    const phaseRef =
      opts?.phase != null
        ? { index: this.ensurePhase(String(opts.phase)), title: String(opts.phase) }
        : this.currentPhase
    ...
    return (await this.sem.run(async () => {
      ...
      let worktree: (Worktree & { gitRoot: string; leaseId?: string }) | undefined
      const runSpec = { ...spec }
      ...
      const transcript = new AgentTranscript(this.o.runId, index, { roots: this.o.roots })
      ...
      try {
        if (opts?.worktree) {
          worktree = this.o.taskflowMode
            ? await this.setupTaskflowLease(runSpec, opts.worktree, key, index)
            : await this.setupWorktree(runSpec, opts.worktree, index)
        }
        ...
        let result = await withRetry(() => worker.runAgent(runSpec, workerCtx), this.o.signal, this.o.retry)
        ...
        try {
          ...
          if (
            ...
            result = await withRetry(() => worker.runAgent(corrective, workerCtx), this.o.signal, this.o.retry)
        ...
        if (worktree?.leaseId && this.o.executionPort) {
          const summary = await this.o.executionPort.summarizeWorkspace(worktree.leaseId).catch(() => null)
          await this.o.executionPort.releaseWorkspace(worktree.leaseId, { discard: false }).catch(() => undefined)
          if (summary?.dirty) {
            this.o.events.emit({
              type: 'agent',
              key,
          ...
          const teardown = await this.worktreeMutex
            .run(() =>
              teardownWorktree({
                gitRoot: activeWorktree.gitRoot,
                worktree: {
                  path: activeWorktree.path,
                  branch: activeWorktree.branch,
                  base: activeWorktree.base,
                },
              }),
            )
            .catch(() => undefined)
  ...
  private async setupTaskflowLease(
    spec: AgentSpec,
    wt: boolean | string,
    executionKey: string,
    index: number,
  ): Promise<Worktree & { gitRoot: string; leaseId: string }> {
    const port = this.o.executionPort
    if (!port) {
      throw new WorkflowError(
        `worktree is unavailable in ${APP_NAME} mode without an ExecutionPort fork lease because ${APP_NAME} owns workspace lifecycle`,
      )
    }
    const acquired = await port.acquireWorkspace({
      kind: 'fork',
      executionKey,
      branchSlug: typeof wt === 'string' ? wt : `agent-${index}`,
    })
    spec.cwd = acquired.cwd
    return {
      path: acquired.cwd,
      branch: acquired.branch,
      base: acquired.baseCommit,
      gitRoot: acquired.cwd,
  ...
  private async setupWorktree(
    ...
    wt: boolean | string,
    index: number,
  ): Promise<Worktree & { gitRoot: string }> {
