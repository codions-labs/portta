...
import { DefaultWorkerFactory } from '../worker/factory.ts'
import type { WorkerFactory } from '../worker/index.ts'
import { type EventListener, FileEventSink } from './event-sink.ts'
...
import { determinismLint, KEY_VERSION } from './keys.ts'
import { checkProvider, checkProviderModelPair, checkSpecEnum, Runtime } from './primitives.ts'
import { TerminalRenderer } from './progress.ts'
import { parseWorkflow, runInSandbox } from './sandbox.ts'
...
export interface RunOptions {
  file: string
  /** Engine-owned id for journal/event storage; distinct from externalContext.runId. */
  runId?: string
  args?: unknown
  ...
  /** Taskflow disables engine-owned worktrees so workspace lifecycle has one owner. */
  mode?: 'standalone' | 'taskflow'
  /** Stable Taskflow identity stored only when the journal is first created. */
  externalContext?: ExternalRunContext
  /** Explicit data/workspace roots for this Run. */
  roots?: RunRoots
  /** Per-Run factory. Host embedders inject this so concurrent Runs do not share workers. */
  factory?: WorkerFactory
  /** Provider ids the factory can serve. Defaults to the builtins; an ACP host passes its registry. */
  knownProviders?: readonly string[]
  /** Taskflow host adapter for fork leases. Engine never imports the backend. */
...
export async function runWorkflow(opts: RunOptions): Promise<RunOutcome> {
  ...
  if (findings.length > 0) {
    throw new Error(`determinism lint failed: ${findings.map((f) => `${f.token} → use ${f.use}`).join('; ')}`)
  }

  const parsed = parseWorkflow(source)
  const defaults = resolveDefaults(parsed.meta, opts)

  const runId = opts.resumeRunId ?? opts.runId ?? opts.externalContext?.runId ?? newRunId()
  ...
  if (opts.resumeRunId) {
    ...
    if (!Journal.exists(runId, opts.roots)) throw new JournalNotFoundError(runId, listRunIds(opts.roots))
    loaded = Journal.load(runId, opts.roots)
  ...
  ensureRunDir(runId, opts.roots)
  opts.onStart?.(runId)
  const journal = new Journal(runId, opts.roots)
  if (!loaded.meta) {
    journal.append({
      type: 'meta',
      runId,
      ...
      ...(opts.externalContext ? { externalContext: opts.externalContext } : {}),
    })
  }
  ...
  const events = new FileEventSink(runId, { listeners, roots: opts.roots })
  ...
  const hostOwnsSignal = opts.signal !== undefined
  if (!hostOwnsSignal) {
  ...
  const heartbeatFile = join(runDir(runId, opts.roots), '.heartbeat')
  const beat = (): void => {
    try {
  ...
  const runtime = new Runtime({
    ...
    defaults,
    factory,
    knownProviders: opts.knownProviders ?? BUILTIN_PROVIDER_IDS,
    journal,
    loaded,
    events,
    args: opts.args,
    ...
    signal: ac.signal,
    roots: opts.roots,
    taskflowMode: opts.mode === 'taskflow',
    executionPort: opts.executionPort,
    declaredPhases: parsed.meta.phases,
  })
  try {
    ...
    writeResult(runId, result ?? null, opts.roots)
  } catch (err) {
...
function resolveDefaults(
  meta: { defaultProvider?: ProviderId; defaultModel?: string; defaultSandbox?: Sandbox },
  opts: RunOptions,
): RunDefaults {
  ...
  checkProvider(provider, opts.knownProviders ?? BUILTIN_PROVIDER_IDS)
  ...
  // (parseWorkflow validates the meta site too, so `taskflow validate` catches it without a run.)
  ...
  const configuredCwd = o.cwd ?? opts.roots?.workspaceRoot
  if (opts.mode === 'taskflow' && !configuredCwd) {
    throw new Error('Taskflow mode requires roots.workspaceRoot instead of daemon cwd')
  }
  return {
    provider,
    ...
    cwd: resolve(configuredCwd ?? process.cwd()),
    concurrency,
