...
import type { Meta, WorkflowGlobals } from '../dsl/types.ts'
...
function validateMeta(v: unknown): asserts v is Meta {
  ...
  if (typeof o.name !== 'string' || o.name.length === 0)
    throw new WorkflowSyntaxError('meta.name must be a non-empty string')
  ...
  // elsewhere — a model meant for a different provider. Failing here means `taskflow validate`
  ...
  if (o.workspace !== undefined) {
    if (typeof o.workspace !== 'object' || o.workspace === null || Array.isArray(o.workspace)) {
      throw new WorkflowSyntaxError('meta.workspace must be an object')
    }
    const workspace = o.workspace as Record<string, unknown>
    const strategies = ['isolated_worktree', 'new_branch', 'current_branch']
    if (!strategies.includes(String(workspace.default))) {
      throw new WorkflowSyntaxError(`meta.workspace.default must be one of ${strategies.join(', ')}`)
    }
    if (workspace.allowed !== undefined) {
      if (
        !Array.isArray(workspace.allowed) ||
        workspace.allowed.length === 0 ||
        workspace.allowed.some((value) => !strategies.includes(String(value)))
      ) {
        throw new WorkflowSyntaxError(
          `meta.workspace.allowed must be a non-empty array containing only ${strategies.join(', ')}`,
        )
      }
      if (!workspace.allowed.includes(workspace.default)) {
        throw new WorkflowSyntaxError('meta.workspace.allowed must include meta.workspace.default')
      }
    }
    if (workspace.mutatesRepository !== undefined && typeof workspace.mutatesRepository !== 'boolean') {
      throw new WorkflowSyntaxError('meta.workspace.mutatesRepository must be a boolean')
    }
    if (
      workspace.reason !== undefined &&
      (typeof workspace.reason !== 'string' || workspace.reason.trim().length === 0)
    ) {
      throw new WorkflowSyntaxError('meta.workspace.reason must be a non-empty string')
    }
  }
}
