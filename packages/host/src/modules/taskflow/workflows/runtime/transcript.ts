...
export function agentsDir(runId: string, roots: RunRoots = {}): string {
  return join(runDir(runId, roots), 'agents')
}

export function agentTranscriptPath(runId: string, index: number, roots: RunRoots = {}): string {
  return join(agentsDir(runId, roots), `${index}.jsonl`)
}

/** Path for a transcript named by an opaque file stem (e.g. a journal key) rather than an index. */
export function agentTranscriptPathByName(runId: string, name: string, roots: RunRoots = {}): string {
  return join(agentsDir(runId, roots), `${sanitizeName(name)}.jsonl`)
}
...
export class AgentTranscript {
  ...
  constructor(runId: string, index: number, opts: AgentTranscriptOpts = {}) {
    const path = opts.name
      ? agentTranscriptPathByName(runId, opts.name, opts.roots)
      : agentTranscriptPath(runId, index, opts.roots)
