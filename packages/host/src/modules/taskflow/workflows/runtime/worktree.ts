...
import { APP_NAME, RUNTIME_IDENTITY } from 'portta-core/taskflow/config'
import { projectPaths } from 'portta-core/taskflow/paths'

const exec = promisify(execFile)

const BASE_CONFIG_KEY = RUNTIME_IDENTITY.gitBaseConfigKey
...
export async function createWorktree(args: {
  ...
  const path = join(projectPaths(args.gitRoot).worktrees, name)
  ...
  // `--relative-paths` keeps the worktree's `.git` link relative. An absolute link is a host path
  // that does not exist inside a Dev Container, and Git there fails with "not a git repository".
  // The project worktree creator (adapters/git.ts) uses the same policy.
  await git(args.gitRoot, ['worktree', 'add', '--relative-paths', '--detach', path, base])
  ...
  try {
    await git(args.gitRoot, ['worktree', 'lock', path, '--reason', `${APP_NAME} ${name}`])
  } catch {
    // lock is best-effort
  }
