...
import { readdir, readFile, realpath, stat, watch } from 'node:fs/promises'
import type { ServerResponse as HttpServerResponse } from 'node:http'
import { createServer, type IncomingMessage, type ServerResponse } from 'node:http'
import { basename, dirname, extname, join, normalize, sep } from 'node:path'
import { fileURLToPath } from 'node:url'
import type { ProviderId } from '../dsl/types.ts'
import type { AgentState, WorkflowEvent } from '../runtime/events.ts'
import { dataRoot } from '../runtime/journal.ts'
import type { ChatChunk } from '../runtime/transcript.ts'
import { nearestExistingDir, parseLastEventId, readNewLines } from './tail.ts'
...
const WEB_DIR = WEB_CANDIDATES.find((path) => existsSync(path)) ?? join(__dirname, 'web')
...
function foldSnapshot(runId: string, events: WorkflowEvent[], lastBeat?: number, raw = false): RunSnapshot {
  ...
  const name = workflowFile
    ? basename(workflowFile)
        .replace(/\.workflow\.[cm]?[jt]s$/i, '')
        .replace(/\.[cm]?[jt]s$/i, '')
    : undefined
...
async function listRuns(): Promise<RunSummary[]> {
  ...
  for (const ent of entries) {
    ...
    if (!rawSummary) {
      ...
      if (eventsStat)
        summaryCache.set(runId, { size: eventsStat.size, mtimeMs: eventsStat.mtimeMs, summary: rawSummary })
    }
