...
import type { Approval, Effort, JSONSchema, Sandbox } from '../dsl/types.ts'
...
// ---------------------------------------------------------------------------
// Result payload shapes we read
// ---------------------------------------------------------------------------

/** Read the provider thread id from a thread/start result, tolerating the
 *  several shapes the app-server has used (`thread.id`, `threadId`, `providerThreadId`). */
 ...
 *  return its `userAgent`: "<originator>/<server version> (<os>) <terminal> (<client name>; <client version>)".
 *  It is the response's one required field and the protocol's only version
...
export function isTokenUsage(params: unknown): params is { threadId: string; tokenUsage: Record<string, unknown> } {
...
export function isTurnCompleted(params: unknown): params is { threadId: string; turn: Record<string, unknown> } {
