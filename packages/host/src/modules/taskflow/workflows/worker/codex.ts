...
import { APP_NAME, APP_SLUG, CLI_NAME } from 'portta-core/taskflow/config'

import type { AgentResult, AgentSpec, AgentUsage } from '../dsl/types.ts'
import { emptyUsage } from '../dsl/types.ts'
import {
  codexErrorCode,
  encodeNotification,
  encodeRequest,
...
import { JsonRpcResponseError, JsonRpcStdioClient, type SpawnChild, StdioTransportError } from './jsonrpc-stdio.ts'
import { parseJsonLoose, toCodexOutputSchema } from './schema.ts'
...
export class CodexWorker implements Worker {
  ...
  private async startAndHandshake(): Promise<void> {
    ...
    const initParams: InitializeParams = {
      clientInfo: { name: APP_SLUG, version: '0.0.1' },
    ...
    if (!userAgent) {
      throw new AgentError({
        ...
        message: `codex app-server returned an initialize result without a userAgent (protocol mismatch?) — upgrade the codex CLI or pin one compatible with ${APP_NAME}`,
        retryable: false,
      })
    }
  ...
  private handleNotification(method: string, params: unknown): void {
    ...
    switch (method) {
      ...
      case 'item/reasoning/textDelta': {
        if (!isThreadDelta(params)) return
        const state = this.turns.get(params.threadId)
        if (state?.forwardProgress) state.ctx.onProgress({ kind: 'reasoning', text: params.delta })
        return
      }
      case 'item/started': {
  ...
  private handleImageGeneration(state: TurnState, item: Record<string, unknown>): void {
    ...
    const write = (async () => {
      try {
        ...
        if (state.forwardProgress)
          state.ctx.onProgress({ kind: 'tool-result', id, name: 'image_generation', output: `saved image → ${dest}` })
      } catch {
  ...
  private onTurnCompleted(p: { threadId: string; turn: Record<string, unknown> }): void {
    ...
    const message =
      (isObject(turnError) && typeof turnError.message === 'string' && turnError.message) ||
  ...
  private onTurnCompletedDrift(params: unknown): void {
    const err = new AgentError({
      provider: PROVIDER,
      code: 'protocol_drift',
      message: `codex sent turn/completed in an unrecognized shape (server: ${this.serverUserAgent ?? 'unknown'}) — upgrade ${CLI_NAME} or pin a compatible codex CLI`,
      retryable: false,
    })
    ...
    if (!client) {
      return Promise.reject(
        new AgentError({
          provider: PROVIDER,
          code: 'process_exited',
          message: 'codex transport is gone',
          retryable: true,
        }),
      )
    }
