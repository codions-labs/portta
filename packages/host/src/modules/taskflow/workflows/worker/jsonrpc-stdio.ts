...
export class JsonRpcStdioClient {
  ...
  start(): void {
    ...
    child.on('error', (err) => this.handleProcessGone(new StdioTransportError('process_error', errMessage(err))))
    ...
    return new Promise<unknown>((resolve, reject) => {
      ...
      if (this.requestTimeoutMs > 0) {
        entry.timer = setTimeout(() => {
          ...
          reject(
            new StdioTransportError('request_timeout', `request ${method} timed out after ${this.requestTimeoutMs}ms`),
          )
