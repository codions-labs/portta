...
import { type AgentResult, type AgentSpec, type AgentUsage, addUsage, emptyUsage } from '../dsl/types.ts'
  ...
  async runAgent(spec: AgentSpec, ctx: WorkerContext): Promise<AgentResult> {
    ...
    if (spec.maxTurns !== undefined) {
      throw new AgentError({
        ...
        message:
          'opencode has no enforceable turn cap (its `steps` config is advisory); omit maxTurns or use the claude-code provider',
      })
    }
    ...
    if (spec.approval !== 'never') {
      throw new AgentError({
        provider: PROVIDER,
        code: 'unsupported_option',
        message: `opencode runs as a one-shot subprocess and cannot surface approval requests to ${APP_NAME} — use approval: "never" with provider "opencode"`,
      })
    }
  ...
  private async runTurn(
    ...
    if (text.length === 0) {
      throw new AgentError({
        provider: PROVIDER,
        code: 'no_result',
        message: 'opencode exited 0 without producing any assistant text',
      })
    }
