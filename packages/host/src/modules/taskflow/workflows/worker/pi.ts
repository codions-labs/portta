...
import { APP_NAME, APP_SLUG } from 'portta-core/taskflow/config'
import { type AgentResult, type AgentSpec, type AgentUsage, addUsage, type Effort, emptyUsage } from '../dsl/types.ts'
import type { Worker, WorkerContext, WorkerProgress } from './index.ts'
import { AgentError, AgentInterrupted } from './index.ts'
import { assertValidSchema, parseJsonLoose } from './schema.ts'
import {
  captureStdout,
...
/** taskflow effort → pi --thinking (pi clamps to the model's supported levels itself). */
...
export class PiWorker implements Worker {
  ...
  async runAgent(spec: AgentSpec, ctx: WorkerContext): Promise<AgentResult> {
    ...
    if (spec.maxTurns !== undefined) {
      throw new AgentError({
        ...
        message:
          'pi has no native turn cap (the agent loop runs until no tool calls remain); omit maxTurns or use the claude-code provider',
      })
    }
    ...
    if (spec.approval !== 'never') {
      throw new AgentError({
        provider: PROVIDER,
        code: 'unsupported_option',
        message: `pi runs as a one-shot subprocess and cannot surface approval requests to ${APP_NAME} — use approval: "never" with provider "pi"`,
      })
    }
  ...
  private baseArgs(spec: AgentSpec): string[] {
    return [
      '--mode',
      'json',
      // taskflow owns history/resume — never litter ~/.pi/agent/sessions.
      '--no-session',
  ...
  private async checkVersion(): Promise<void> {
    ...
    const scratch = mkdtempSync(join(tmpdir(), `${APP_SLUG}-pi-version-`))
    let out: string
    try {
  ...
  private async runTurn(
    ...
    const exit = await runJsonlSubprocess({
      ...
      env: { ...process.env, ...spec.launcherEnv },
      stdin: prompt,
      signal: ctx.signal,
        ...
        switch (value.type) {
          ...
          case 'message_end': {
            ...
            if (u) {
              ...
              usage = {
                inputTokens:
                  usage.inputTokens + (numOf(u.input) ?? 0) + (numOf(u.cacheRead) ?? 0) + (numOf(u.cacheWrite) ?? 0),
    ...
    if (ctx.signal.aborted) throw new AgentInterrupted()
    if (sawAborted) {
      // pi aborted internally without taskflow asking — not a success, not our interrupt.
      throw new AgentError({
        provider: PROVIDER,
        code: 'aborted',
        message: 'pi aborted the request',
        retryable: false,
      })
    }
    ...
    if (text.length === 0) {
      throw new AgentError({
        provider: PROVIDER,
        code: 'no_result',
        message: 'pi exited 0 without producing any assistant text',
      })
    }
    return { text, usage }
  }
