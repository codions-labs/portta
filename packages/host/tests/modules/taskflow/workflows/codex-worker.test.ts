...
} from '../../../../src/modules/taskflow/workflows/worker/codex.ts'
import {
  AgentError,
  AgentInterrupted,
  type WorkerProgress,
...
import {
  ...
  JsonRpcStdioClient,
  StdioTransportError,
} from '../../../../src/modules/taskflow/workflows/worker/jsonrpc-stdio.ts'
...
interface FakeRpcMessage extends Record<string, unknown> {
  id?: unknown
  method?: string
...
function fakeStream(): EventEmitter & { setEncoding(encoding: string): void } {
  ...
  return Object.assign(emitter, { setEncoding: (_encoding: string): void => {} })
}
...
class FakeChild extends EventEmitter {
  ...
  onWrite?: (obj: FakeRpcMessage) => void
          ...
          for (const line of chunk.split('\n')) {
            ...
            try {
              ...
              if (typeof parsed === 'object' && parsed !== null && !Array.isArray(parsed)) {
                self.onWrite?.(parsed as FakeRpcMessage)
              }
            } catch {
              // ignore non-JSON
            }
...
function asChildProcess(child: FakeChild): ChildProcessWithoutNullStreams {
...
test('JsonRpcStdioClient: request resolves on matching response', async () => {
  ...
  const client = new JsonRpcStdioClient({ spawnChild: () => asChildProcess(child) })
  client.start()
...
test('H1/M1: process exit rejects all pending requests and resets buffer', async () => {
  ...
  const client = new JsonRpcStdioClient({
    ...
    onProcessGone: (error) => (gone = error),
  })
  client.start()
  ...
  assert.throws(
    () => client.send('x'),
    (e) => e instanceof StdioTransportError && e.code === 'not_writable',
  )
...
test('M2: stderr is drained into a bounded ring buffer', () => {
  const child = new FakeChild()
  const client = new JsonRpcStdioClient({ spawnChild: () => asChildProcess(child), stderrLimit: 10 })
  client.start()
...
test('M30: request timeout rejects a wedged request', async () => {
  const child = new FakeChild()
  const client = new JsonRpcStdioClient({ spawnChild: () => asChildProcess(child), requestTimeoutMs: 30 })
  client.start()
...
function makeServedWorker(
  turnScript: (req: FakeRpcMessage, reply: (obj: unknown) => void, turnIndex: number) => void,
  opts: {
    requestTimeoutMs?: number
    turnStallTimeoutMs?: number
    threadId?: string
    initResult?: unknown
    onServerReq?: (child: FakeChild, req: FakeRpcMessage) => void
  } = {},
    ...
    spawnChild: () => {
      ...
      child.onWrite = (req) => {
        if (req.method === 'initialize')
          return child.pushLine({ jsonrpc: '2.0', id: req.id, result: opts.initResult ?? INIT_OK })
        if (req.method === 'initialized') return
        if (req.method === 'thread/start')
          return child.pushLine({ jsonrpc: '2.0', id: req.id, result: { thread: { id: threadId } } })
...
test('runAgent happy path (served before spawn): resolves with usage', async () => {
  const { worker } = makeServedWorker((_req, reply) => {
    ...
    reply({
      ...
      params: {
        threadId: 'thread-1',
        tokenUsage: { total: { inputTokens: 50, outputTokens: 10 }, last: { inputTokens: 50, outputTokens: 10 } },
      },
    })
    reply({
      jsonrpc: '2.0',
...
test("H5: schema agent reports the extraction turn's cumulative total exactly once", async () => {
  ...
  const { worker } = makeServedWorker((_req, reply, idx) => {
    if (idx === 0) {
      reply({
        ...
        params: {
          threadId: 'thread-1',
          tokenUsage: { total: { inputTokens: 60, outputTokens: 10 }, last: { inputTokens: 60, outputTokens: 10 } },
        },
      })
      reply({
        jsonrpc: '2.0',
        method: 'item/completed',
        params: { threadId: 'thread-1', item: { type: 'agentMessage', text: 'working' } },
      })
      reply({
        jsonrpc: '2.0',
        method: 'thread/tokenUsage/updated',
        params: {
          threadId: 'thread-1',
          tokenUsage: { total: { inputTokens: 100, outputTokens: 20 }, last: { inputTokens: 40, outputTokens: 10 } },
        },
      })
      reply({
        jsonrpc: '2.0',
      ...
      reply({
        jsonrpc: '2.0',
        method: 'item/completed',
        params: { threadId: 'thread-1', item: { type: 'agentMessage', text: '{"answer": 42}' } },
      })
      reply({
        jsonrpc: '2.0',
        method: 'thread/tokenUsage/updated',
        params: {
          threadId: 'thread-1',
          tokenUsage: { total: { inputTokens: 130, outputTokens: 25 }, last: { inputTokens: 30, outputTokens: 5 } },
        },
      })
      reply({
        jsonrpc: '2.0',
  ...
  const res = await worker.runAgent(
  ...
  const { worker } = makeServedWorker((_req, reply, idx) => {
    if (idx === 0) {
      reply({
        ...
        params: {
          threadId: 'thread-1',
          tokenUsage: { total: { inputTokens: 100, outputTokens: 20 }, last: { inputTokens: 100, outputTokens: 20 } },
        },
      })
      reply({
        jsonrpc: '2.0',
      ...
      reply({
        jsonrpc: '2.0',
        method: 'item/completed',
        params: { threadId: 'thread-1', item: { type: 'agentMessage', text: '{"answer": 1}' } },
      })
...
test('H5: non-schema single turn reports its final cumulative total (multi-request turn)', async () => {
  const { worker } = makeServedWorker((_req, reply) => {
    reply({
      ...
      params: {
        threadId: 'thread-1',
        tokenUsage: { total: { inputTokens: 10, outputTokens: 2 }, last: { inputTokens: 10, outputTokens: 2 } },
      },
    })
    reply({
      jsonrpc: '2.0',
      ...
      params: {
        threadId: 'thread-1',
        tokenUsage: { total: { inputTokens: 35, outputTokens: 9 }, last: { inputTokens: 25, outputTokens: 7 } },
      },
    })
    reply({
      jsonrpc: '2.0',
      method: 'item/completed',
      params: { threadId: 'thread-1', item: { type: 'agentMessage', text: 'ok' } },
    })
    reply({
      jsonrpc: '2.0',
      method: 'turn/completed',
...
test('H2: an `error` notification (no turn/completed) rejects the turn', async () => {
  ...
  await assert.rejects(
    ...
    (e) => e instanceof AgentError && /model exploded/.test(e.message) && e.retryable === true,
  )
  await worker.shutdown()
})
...
test('H2: error notification without threadId settles all live turns', async () => {
  ...
  await assert.rejects(
    worker.runAgent(spec(), ctx()),
    (e) => e instanceof AgentError && /global failure/.test(e.message),
  )
  await worker.shutdown()
})
...
test('H1: child crash mid-turn rejects runAgent (no hang)', async () => {
  ...
  const worker = new CodexWorker({
    spawnChild: () => {
      ...
      return asChildProcess(theChild)
    },
  })
  ...
  const worker = new CodexWorker({
    spawnChild: () => {
      ...
      child.onWrite = (req) => {
        ...
        if (req.method === 'thread/start')
          return child.pushLine({ jsonrpc: '2.0', id: req.id, result: { thread: { id: `t${spawnCount}` } } })
        if (req.method === 'turn/start') {
          child.pushLine({ jsonrpc: '2.0', id: req.id, result: {} })
          if (isFirst) {
            ...
            child.pushLine({
              jsonrpc: '2.0',
              method: 'item/completed',
              params: { threadId: 't2', item: { type: 'agentMessage', text: 'recovered' } },
            })
            child.pushLine({
              jsonrpc: '2.0',
              method: 'turn/completed',
              params: { threadId: 't2', turn: { status: 'completed' } },
            })
          }
        }
      }
  ...
  const worker = new CodexWorker({
    spawnChild: () => {
      ...
      child.onWrite = (req) => {
        ...
        if (req.method === 'initialize') return child.pushLine({ jsonrpc: '2.0', id: req.id, result: INIT_OK })
        if (req.method === 'thread/start')
          return child.pushLine({ jsonrpc: '2.0', id: req.id, result: { thread: { id: 't' } } })
        if (req.method === 'turn/start') {
          child.pushLine({ jsonrpc: '2.0', id: req.id, result: {} })
          ...
          child.pushLine({
            jsonrpc: '2.0',
            method: 'item/completed',
            params: { threadId: 't', item: { type: 'agentMessage', text: 'x' } },
          })
          child.pushLine({
            jsonrpc: '2.0',
            method: 'turn/completed',
...
test('H3: read-only declines command approval; non-read-only accepts (capture writes)', async () => {
  assert.deepEqual(await approvalDecision('read-only', 'item/commandExecution/requestApproval'), {
    decision: 'decline',
  })
  assert.deepEqual(await approvalDecision('read-only', 'item/fileChange/requestApproval'), { decision: 'decline' })
  assert.deepEqual(await approvalDecision('workspace-write', 'item/commandExecution/requestApproval'), {
    decision: 'accept',
  })
  assert.deepEqual(await approvalDecision('workspace-write', 'item/fileChange/requestApproval'), {
    decision: 'accept',
  })
})
...
test('H3: permissions approval grants nothing for ANY sandbox (wire-level reply shape)', async () => {
  ...
  assert.deepEqual(await approvalDecision('read-only', 'item/permissions/requestApproval'), {
    permissions: {},
    scope: 'turn',
  })
  assert.deepEqual(await approvalDecision('workspace-write', 'item/permissions/requestApproval'), {
    permissions: {},
    scope: 'turn',
  })
  assert.deepEqual(await approvalDecision('danger-full-access', 'item/permissions/requestApproval'), {
    permissions: {},
    scope: 'turn',
  })
})
...
test('M3: imageGeneration writes to cwd, sanitizes id, and is awaited before settle', async () => {
  ...
  const { worker } = makeServedWorker((_req, reply) => {
    ...
    reply({
      jsonrpc: '2.0',
      method: 'item/completed',
      params: { threadId: 'thread-1', item: { type: 'agentMessage', text: 'made an image' } },
    })
    reply({
      jsonrpc: '2.0',
      method: 'turn/completed',
...
test('M30: malformed notifications are ignored, turn still completes', async () => {
  const { worker } = makeServedWorker((_req, reply) => {
    ...
    reply({
      jsonrpc: '2.0',
      method: 'item/completed',
      params: { threadId: 'thread-1', item: { type: 'agentMessage', text: 'survived' } },
    })
    reply({
      jsonrpc: '2.0',
      method: 'turn/completed',
...
test('M30: production construction (factory passes no timeout opts) arms BOTH watchdogs', () => {
  ...
  assert.equal(Reflect.get(worker, 'requestTimeoutMs'), DEFAULT_REQUEST_TIMEOUT_MS)
  assert.equal(Reflect.get(worker, 'turnStallTimeoutMs'), DEFAULT_TURN_STALL_TIMEOUT_MS)
  ...
  assert.equal(Reflect.get(off, 'requestTimeoutMs'), 0)
  assert.equal(Reflect.get(off, 'turnStallTimeoutMs'), 0)
})
...
test('M30: notification activity re-arms the stall watchdog (a slow turn with steady progress completes)', async () => {
  // Total turn time (~120ms) exceeds the stall window (80ms), but no
  ...
  const { worker } = makeServedWorker(
    (_req, reply) => {
      ...
      setTimeout(() => delta('a'), 30)
      setTimeout(() => delta('b'), 60)
      setTimeout(() => delta('c'), 90)
      setTimeout(
        () =>
          reply({
            jsonrpc: '2.0',
...
test('M30/H3: inbound approval REQUESTS re-arm the stall watchdog (approval-only traffic keeps a turn alive)', async () => {
  ...
  // a healthy turn mid-approval. Total turn time (~120ms) exceeds the stall
  // window (80ms); no inter-frame gap does.
  const { worker, getChild } = makeServedWorker(
    (_req, reply) => {
      ...
      setTimeout(() => approval(101), 30)
      setTimeout(() => approval(102), 60)
      setTimeout(() => approval(103), 90)
      setTimeout(
        () =>
          reply({
            jsonrpc: '2.0',
    ...
    (_req, reply) => {
      const junk = () =>
        reply({
          ...
          params: { threadId: 'thread-1', textDelta: 'drifted' },
        })
      setTimeout(junk, 30)
      setTimeout(junk, 60)
      setTimeout(junk, 90)
...
test('M30: drifted turn/completed (no `turn` member) settles the turn with protocol_drift — not a hang', async () => {
  ...
  await assert.rejects(
    worker.runAgent(spec(), ctx()),
    (e) =>
      e instanceof AgentError && e.code === 'protocol_drift' && e.retryable === false && /0\.0\.0-test/.test(e.message),
  )
  await worker.shutdown()
})
...
test('M30: a pre-v2 app-server (initialize ok, thread/start unknown) fails loudly — behavioral version negotiation', async () => {
  ...
  const worker = new CodexWorker({
    spawnChild: () => {
      ...
      child.onWrite = (req) => {
        if (req.method === 'initialize')
          return child.pushLine({ jsonrpc: '2.0', id: req.id, result: { userAgent: 'codex/0.20.0 (old)' } })
        if (req.method === 'thread/start')
          return child.pushLine({
            jsonrpc: '2.0',
            id: req.id,
            error: { code: -32601, message: 'Method not found: thread/start' },
          })
      }
      return asChildProcess(child)
    },
...
test('turn/completed with status=failed → AgentError with codex code + retryable', async () => {
  const { worker } = makeServedWorker((_req, reply) => {
    reply({
      ...
      params: {
        threadId: 'thread-1',
        turn: { status: 'failed', error: { message: 'overloaded', codexErrorInfo: { serverOverloaded: {} } } },
      },
    })
  })
  await assert.rejects(
...
test('pre-aborted signal throws AgentInterrupted before spawning', async () => {
  ...
  ac.abort()
  const worker = new CodexWorker({ spawnChild: () => asChildProcess(new FakeChild()) })
