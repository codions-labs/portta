...
import { emptyUsage } from '../../../../src/modules/taskflow/workflows/dsl/types.ts'
import {
...
function withHome<T>(fn: (home: string) => T): T {
  const home = mkdtempSync(join(tmpdir(), 'taskflow-journal-'))
...
test('checkResumePreconditions rejects a stale key version', () => {
  assert.throws(
    () =>
      checkResumePreconditions(
        { ...baseMeta, keyVersion: 'v1' },
        { fileHash: 'HASH', args: { topic: 'x' }, keyVersion: 'v2' },
      ),
    ResumePreconditionError,
  )
})

test('a journal meta with NO keyVersion field is rejected on resume — absent means v1, not skip-the-check', () => {
  // Treating an absent keyVersion as "nothing to compare" would let the journal resume under the
  // current scheme with 100% key misses — a full silent re-bill (C1).
...
test('runDir / journalPath / ensureRunDir use PORTTA_HOST_STATE_DIR', () => {
  withHome((home) => {
