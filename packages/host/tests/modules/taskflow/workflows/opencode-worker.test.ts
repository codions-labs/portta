...
import { OPENCODE_MIN_VERSION, OpencodeWorker } from '../../../../src/modules/taskflow/workflows/worker/opencode.ts'
...
test('schema extraction turn preserves effort variant', async () => {
  ...
  await h.worker.runAgent(
    spec({ effort: 'max', schema: { type: 'object', properties: { answer: { type: 'number' } } } }),
    ctx(),
  )
