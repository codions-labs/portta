...
import type { AgentSpec, Effort } from '../../../../src/modules/taskflow/workflows/dsl/types.ts'
import {
  AgentError,
  AgentInterrupted,
  type WorkerProgress,
} from '../../../../src/modules/taskflow/workflows/worker/index.ts'
import { PI_MIN_VERSION, PiWorker } from '../../../../src/modules/taskflow/workflows/worker/pi.ts'
import type { SpawnProcess } from '../../../../src/modules/taskflow/workflows/worker/subprocess-jsonl.ts'
...
test('happy path: argv shape, stdin prompt, delta + tool mapping, usage folding', async () => {
  ...
  const result = await h.worker.runAgent(
    spec({
      ...
      launcherCwd: '/host/project',
      launcherEnv: { PORTTA_FLOW_ENVIRONMENT_CWD: '/tmp/project' },
    }),
    c,
  )
  ...
  assert.equal(h.spawned[1]?.cwd, '/host/project')
  assert.equal(h.spawned[1]?.env?.PORTTA_FLOW_ENVIRONMENT_CWD, '/tmp/project')
...
test("stopReason 'error' is fatal despite exit 0 (json mode hides it from the exit code)", async () => {
  const h = harness([
    ...
    (p) => {
      ...
      p.pushLine({
        type: 'message_end',
        message: assistantMessage('partial', { stopReason: 'error', errorMessage: 'rate limited' }),
      })
      p.end(0)
    },
  ])
    ...
    (p) => {
      p.pushLine({
        ...
        assistantMessageEvent: {
          type: 'error',
          reason: 'error',
          error: assistantMessage('', { stopReason: 'error', errorMessage: 'boom' }),
        },
      })
      p.end(0)
    },
...
test("stopReason 'aborted' without an taskflow abort is a provider failure, not success", async () => {
  const h = harness([
    versionOk,
    (p) => {
...
test('exit 0 with no assistant text is no_result', async () => {
  ...
  await assert.rejects(
    ...
    (err: unknown) => err instanceof AgentError && err.code === 'no_result',
  )
})
...
test('version preflight: isolated probe env, once per worker, outdated refused with upgrade hint', async () => {
  ...
  assert.ok(probe.env?.PI_CODING_AGENT_DIR?.includes('taskflow-pi-version-'))
  ...
  const h = harness([
    ...
    (p) => {
      p.pushLine({
        type: 'message_update',
        message: {},
        assistantMessageEvent: { type: 'text_delta', contentIndex: 0, delta: '{"answer": 42}' },
      })
