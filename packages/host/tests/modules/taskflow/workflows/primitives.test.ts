...
import { FileEventSink } from '../../../../src/modules/taskflow/workflows/runtime/event-sink.ts'
...
function build(
  ...
  const home = mkdtempSync(join(tmpdir(), 'taskflow-prim-'))
...
test('parallel() runs each thunk in its own branch and fans out', async () => {
  const b = build({
    hooks: { run: async (s) => ({ text: s.prompt.toUpperCase(), status: 'completed', usage: emptyUsage() }) },
  })
  try {
...
test('pipeline() threads stages and keys by (item, stage) position', async () => {
  const b = build({
    hooks: { run: async (s) => ({ text: `[${s.prompt}]`, status: 'completed', usage: emptyUsage() }) },
  })
  try {
...
test('parallel() swallows ordinary agent failures as null but keeps siblings', async () => {
  ...
  try {
    const out = (await runBody(
      b,
      `return await parallel([() => agent("ok"), () => agent("bad"), () => agent("ok2")])`,
    )) as unknown[]
...
test('H6: parallel() RETHROWS WorkflowError (budget/cap) instead of nulling it', async () => {
  ...
  const b = build({
    ...
    hooks: {
      run: async (s) => ({ text: s.prompt, status: 'completed', usage: { ...emptyUsage(), outputTokens: 100 } }),
    },
  })
  try {
    await assert.rejects(
...
test('M11: budget is re-checked inside the semaphore slot', async () => {
  ...
  const b = build({
    ...
    hooks: {
      run: async (s) => ({ text: s.prompt, status: 'completed', usage: { ...emptyUsage(), outputTokens: 60 } }),
    },
  })
  try {
...
test("L12: transcripts live at agents/<index>.jsonl for the index the agent's events advertise", async () => {
  ...
  try {
    ...
    const done = b.sink.events.filter((e) => e.type === 'agent' && e.state === 'done') as Array<{
      index: number
      label: string
    }>
...
test('random() varies with the run seed but is stable for the same seed', async () => {
  const run = async (seed: number) => {
    const home = mkdtempSync(join(tmpdir(), 'taskflow-seed-'))
    const prev = process.env.PORTTA_HOST_STATE_DIR
    process.env.PORTTA_HOST_STATE_DIR = home
    try {
      const runtime = new Runtime({
...
test('H14: agent() rejects invalid provider/sandbox/effort/approval values at spec resolution', async () => {
  ...
  try {
    await assert.rejects(
      runBody(b, `return await agent("x", { provider: "open-code" })`),
      /invalid provider "open-code"/,
    )
    ...
    const pi = await runBody(
      b,
      `return await agent("z", { provider: "pi", model: "openrouter/moonshotai/kimi-k2.6", sandbox: "danger-full-access" })`,
    )
...
test('L6: failed-turn usage counts against the budget ceiling end-to-end', async () => {
      ...
      run: async (s) => {
        if (s.prompt === 'a')
          throw new AgentError({
            ...
            code: 'boom',
            message: 'nope',
            usage: { ...emptyUsage(), outputTokens: 10 },
          })
        return { text: s.prompt, status: 'completed', usage: emptyUsage() }
      },
    },
  ...
  try {
    ...
    await assert.rejects(runBody(b, `try { await agent("a") } catch (e) {} return await agent("b")`), /budget/)
...
function makeGitRepo(): string {
  const repo = mkdtempSync(join(tmpdir(), 'taskflow-wt-repo-'))
  const g = (...args: string[]) => execFileSync('git', args, { cwd: repo })
  g('init', '-q')
  g('config', 'user.email', 'taskflow@test')
  g('config', 'user.name', 'taskflow')
...
test('L14: a preserved worktree emits worktreeBranch/worktreePath into events.jsonl', async () => {
  ...
  try {
    ...
    const out = await runInSandbox({
      body: `return await agent("edit", { worktree: true })`,
      filename: 't.js',
      globals: runtime.globals(),
    })
    ...
    assert.ok(
      typeof wt.worktreePath === 'string' && wt.worktreePath.endsWith(join('.portta', 'worktrees', `${runId}-1`)),
...
test('H10: teardown trusts the THREADED creation base when the shared-config fallback is gone', async () => {
  ...
  const b = build({
    ...
    hooks: {
      run: async (spec) => {
        execFileSync('git', ['config', '--unset', 'taskflow.base'], { cwd: spec.cwd })
  ...
  try {
    ...
    const wtDir = join(repo, '.portta', 'worktrees')
