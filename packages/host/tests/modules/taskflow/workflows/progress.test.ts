...
import { TerminalRenderer } from '../../../../src/modules/taskflow/workflows/runtime/progress.ts'
...
// biome-ignore lint/suspicious/noControlCharactersInRegex: ESC is required to remove ANSI color sequences from captured output.
    ...
    capture((r) => {
      ...
      r.handle(
        agentEvent({
          ...
          state: 'failed',
          error: 'boom',
          worktreeBranch: 'aw/run-2',
          worktreePath: '/tmp/wt/run-2',
        }),
      )
    }),
  )
