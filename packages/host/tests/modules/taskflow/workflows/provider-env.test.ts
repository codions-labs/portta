...
test('pi: overrides.piBin drives a real spawn with the exact argv/stdin contract', posixOnly, async () => {
  const dir = mkdtempSync(join(tmpdir(), 'taskflow-pi-env-'))
  const prev = { PORTTA_HOST_STATE_DIR: process.env.PORTTA_HOST_STATE_DIR, RECORD: process.env.RECORD }
  try {
    ...
    process.env.PORTTA_HOST_STATE_DIR = join(dir, 'home')
    ...
    restoreEnv('PORTTA_HOST_STATE_DIR', prev.PORTTA_HOST_STATE_DIR)
...
test('opencode: OPENCODE_BIN env drives a real spawn with the exact argv/stdin contract', posixOnly, async () => {
  const dir = mkdtempSync(join(tmpdir(), 'taskflow-oc-env-'))
  const prev = {
    PORTTA_HOST_STATE_DIR: process.env.PORTTA_HOST_STATE_DIR,
