...
import {
  ...
  emptyUsage,
  type RunDefaults,
} from '../../../../src/modules/taskflow/workflows/dsl/types.ts'
import type { EventSink, WorkflowEventInput } from '../../../../src/modules/taskflow/workflows/runtime/events.ts'
import {
  Journal,
...
import { Runtime } from '../../../../src/modules/taskflow/workflows/runtime/primitives.ts'
import { runWorkflow } from '../../../../src/modules/taskflow/workflows/runtime/run.ts'
import { runInSandbox } from '../../../../src/modules/taskflow/workflows/runtime/sandbox.ts'
import type { Worker, WorkerContext, WorkerFactory } from '../../../../src/modules/taskflow/workflows/worker/index.ts'
...
class StaggeredWorker implements Worker {
  ...
  constructor(
    delayFor: (prompt: string) => number,
    /** Optional per-call result override (return undefined for the default echo). */
    respond?: (prompt: string) => AgentResult | undefined,
  ) {
  ...
  async runAgent(spec: AgentSpec, _ctx: WorkerContext): Promise<AgentResult> {
    ...
    return (
      this.respond?.(spec.prompt) ?? {
        text: `done:${spec.prompt}`,
        status: 'completed',
        usage: { ...emptyUsage(), outputTokens: 1 },
      }
    )
  }
...
function withHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  const home = mkdtempSync(join(tmpdir(), 'taskflow-resume-'))
  const prev = process.env.PORTTA_HOST_STATE_DIR
  process.env.PORTTA_HOST_STATE_DIR = home
  return fn(home).finally(() => {
    if (prev === undefined) delete process.env.PORTTA_HOST_STATE_DIR
    else process.env.PORTTA_HOST_STATE_DIR = prev
...
test('ACCEPTANCE (C1): parallel() with staggered completion resumes at 100% cache hits', async () => {
  await withHome(async (home) => {
    ...
    const order1 = (p: string): number =>
      p.endsWith('1') ? ({ alpha1: 40, beta1: 30, gamma1: 20, delta1: 10 }[p] ?? 0) : 0
    ...
    const order2 = (p: string): number =>
      p.endsWith('1') ? ({ alpha1: 10, beta1: 20, gamma1: 30, delta1: 40 }[p] ?? 0) : 0
    ...
    const cached = second.sink.events.filter(
      (e) => e.type === 'agent' && e.state === 'done' && (e as { cached?: boolean }).cached,
    )
...
test('the NEW per-branch scheme is INVARIANT to completion order (keys depend on position only)', async () => {
  await withHome(async (home) => {
    ...
    const a = await runOnce(
      ...
      'wf_a',
      PARALLEL_BODY,
      (p) => ({ alpha: 40, beta: 30, gamma: 20, delta: 10 })[p] ?? 0,
      false,
    )
    const b = await runOnce(
      ...
      'wf_b',
      PARALLEL_BODY,
      (p) => ({ alpha: 10, beta: 20, gamma: 30, delta: 40 })[p] ?? 0,
      false,
    )
...
async function runFile(
  file: string,
  opts: { resume?: string; args?: unknown; concurrency?: number } = {},
): Promise<E2E> {
...
test('M21: resuming an unknown run id fails loudly and names known runs', async () => {
  await withHome(async (home) => {
    ...
    await assert.rejects(
      ...
      (e: unknown) =>
        e instanceof JournalNotFoundError && e.runId === 'wf_typo' && e.message.includes(real.outcome.runId),
    )
...
await new Promise(() => {})`,
    ...
    const guard = new Promise<never>((_, reject) => {
      const t = setTimeout(
        () => reject(new Error('runWorkflow hung after abort — the signal is not wired into runInSandbox (M13)')),
        10_000,
      )
      t.unref()
    })
...
test('rec #4: a journal meta WITHOUT keyVersion is read as v1 and rejected on resume', async () => {
  await withHome(async (home) => {
    const file = join(home, 'wf.js')
    writeFileSync(file, WF)
    const first = await runFile(file)
    // Strip keyVersion from the journal meta. An absent field is read as v1; resuming it under the
    // current scheme would silently miss 100% of keys and re-bill the whole run (the C1 failure mode).
    const loaded = Journal.load(first.outcome.runId)
    const { keyVersion: _dropped, ...v1Meta } = loaded.meta!
    const { journalPath } = await import('../../../../src/modules/taskflow/workflows/runtime/journal.ts')
...
test('--provider/--model overrides are both-or-neither, before the run dir exists', async () => {
  ...
  await withHome(async (home) => {
    ...
    const both = await runWorkflow({
      ...
      fake: true,
      quiet: true,
      overrides: { provider: 'claude-code', model: 'claude-fable-5' },
    })
