...
import type { WorkflowGlobals } from '../../../../src/modules/taskflow/workflows/dsl/types.ts'
import {
  parseWorkflow,
  runInSandbox,
...
} from '../../../../src/modules/taskflow/workflows/runtime/sandbox.ts'
...
test('meta.defaultProvider/defaultModel are both-or-neither at parse time', () => {
  ...
  // — a model meant for a different provider. `taskflow validate` must catch it without a run.
  const lead = `export const meta = { name: "n", description: "d", `
  assert.throws(
    () => parseWorkflow(`${lead}defaultProvider: "claude-code" }\nreturn 1\n`),
    /defaultProvider and meta\.defaultModel must be specified together/,
  )
  assert.throws(
    () => parseWorkflow(`${lead}defaultModel: "gpt-5.5" }\nreturn 1\n`),
    /defaultProvider and meta\.defaultModel must be specified together/,
  )
  ...
  assert.equal(
    parseWorkflow(`export const meta = { name: "n", description: "d" }\nreturn 1\n`).meta.defaultProvider,
    undefined,
  )
})

test('meta.workspace validates recommendations and technical restrictions', () => {
  const parsed = parseWorkflow(`export const meta = {
    name: "review",
    description: "review code",
    workspace: { default: "current_branch", allowed: ["current_branch", "isolated_worktree"], mutatesRepository: false }
  }\nreturn 1\n`)
  assert.equal(parsed.meta.workspace?.default, 'current_branch')
  assert.equal(parsed.meta.workspace?.mutatesRepository, false)
  assert.throws(
    () =>
      parseWorkflow(
        `export const meta = { name: "bad", description: "bad", workspace: { default: "current_branch", allowed: ["isolated_worktree"] } }\nreturn 1\n`,
      ),
    /must include meta\.workspace\.default/,
  )
})
...
test('a non-array meta.phases fails at parse time, not as a TypeError mid-run', () => {
  ...
  assert.throws(
    () => parseWorkflow(`export const meta = { name: "n", description: "d", phases: "Plan" }\n`),
    WorkflowSyntaxError,
  )
