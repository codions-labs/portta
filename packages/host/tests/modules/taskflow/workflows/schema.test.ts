...
import type { JSONSchema } from '../../../../src/modules/taskflow/workflows/dsl/types.ts'
import {
  assertValidSchema,
  parseJsonLoose,
  stripNullOptionals,
  toCodexOutputSchema,
  validate,
} from '../../../../src/modules/taskflow/workflows/worker/schema.ts'
...
test('M6: optional enum field gets null member', () => {
  ...
  assert.ok(((strict.properties as Record<string, JSONSchema>).color!.enum as unknown[] | undefined)?.includes('red'))
})
...
test('strictify recurses into nested objects and arrays', () => {
  const schema: JSONSchema = {
    ...
    properties: {
      list: {
        type: 'array',
        items: { type: 'object', properties: { a: { type: 'string' }, b: { type: 'number' } }, required: ['a'] },
      },
    },
    required: ['list'],
  }
...
test('stripNullOptionals recurses into arrays and nested objects', () => {
  const schema: JSONSchema = {
    ...
    properties: {
      items: {
        type: 'array',
        items: { type: 'object', properties: { x: { type: 'number' }, y: { type: 'string' } }, required: ['x'] },
      },
    },
    required: ['items'],
  }
