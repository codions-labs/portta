// Bake-off: both providers implement the same task in isolated fork leases, then blind judges
// from both providers score the two diffs (neutral A/B labels, shuffled per run, so neither judge
// knows whose work is whose), a tie-break settles splits, and the closing report says what to
// graft from the loser. Both worktree branches are preserved for the user.
// Run with: portta flow run workflow bake-off --input '<task>'
//       or: portta flow workflows run bake-off --args '"<task>"'   (or --args '{"task": "..."}')
...
export const meta = {
  ...
  workspace: {
    ...
    reason: "The checkout is only the stable base; Taskflow creates one isolated child worktree for each contender.",
  },
  phases: [
