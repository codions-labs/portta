// Multi-provider review: the same change reviewed twice — once by Codex, once by Claude — each
// over the ENTIRE feature/branch, then a synthesis pass merges both: consensus findings (both
// providers saw it → highest confidence), unique catches, and disagreements called out.
// Run with: portta flow run workflow multi-provider-review [--input-json '{"target": "<ref|path|diff>"}']
//       or: portta flow workflows run multi-provider-review [--args '{"target": "<ref|path|diff>"}']
...
export const meta = {
  ...
  workspace: {
    ...
    reason: "Both reviewers inspect the active checkout read-only.",
  },
  phases: [
