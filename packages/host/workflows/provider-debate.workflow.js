// Provider debate: one provider proposes a position, the other attacks it, the proposer rebuts
// or concedes, for N rounds — then a judge rules on what actually survived. The cross-provider
// attacker is the point: a model is measurably worse at finding holes in its own reasoning than
// another model is.
// Run with: portta flow run workflow provider-debate --input '<question>'
//       or: portta flow workflows run provider-debate --args '"<question>"'
...
export const meta = {
  ...
  workspace: {
    ...
    reason: "The debate is read-only and normally needs no Git isolation.",
  },
  phases: [
